...
- **`cursorShapes` still requires a restart**: The bundled vim extension receives the shapes at construction (`createBundledVimExtension`), and unlike the settings above it is not gated — it is always present, so slot membership cannot fix it. The fork reads `state.vim.cursorShapes` live on every render (`block-cursor.ts`), and `vim({ cursorShapes })` stores the object by reference, so no fork change is needed: the two `{ ...cursorShapes }` copies in `createBundledVimExtension` are what break the link. Passing the settings object through unchanged would make shape edits apply live, provided the settings layer mutates its properties rather than replacing the object.
