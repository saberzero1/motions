...
- **`vim.fn.getcharstr()`/`vim.fn.getchar()`** are async — they yield the Lua coroutine and resume when a key is pressed. They capture a single keystroke via a capture-phase DOM event listener. Modifier-only keys (Shift, Ctrl, Alt, Meta) are ignored. Cannot be used in expr callbacks or snippet nodes.
