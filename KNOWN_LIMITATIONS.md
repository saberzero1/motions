...
### Neovim messages, prompts, and the command line are not displayed in RPC mode

The attached UI requests only `ext_linegrid`, and every grid event is discarded, so each of Neovim's own UI surfaces has nowhere to draw. `ext_messages`, `ext_cmdline`, and `ext_popupmenu` are all disabled. Three consequences are user-visible:

- `vim.notify()` and `:echo` succeed and reach `:messages`, but nothing is shown. A plugin reporting an error is silent.
...
- Typing `:` opens Neovim's command line with no prompt, no echoed text, and no completion. Commands still execute, but they are typed blind and any resulting error message is also invisible. Bundled-fork mode renders its own command line for this, so the command line is a capability RPC mode currently lacks rather than a shared limitation.
