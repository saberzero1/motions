...
- **Wikilink color loss after cell edit**: After editing a cell containing a wikilink (`[[link]]`) and exiting the cell editor, the wikilink may lose its accent color styling in the re-rendered table widget. The color does not return until the note is reopened or Obsidian is reloaded. Root cause is under investigation — likely related to `MarkdownRenderer.render()` async post-processing timing or Obsidian's metadata cache invalidation after cell write-back. A deferred `Component.unload()` (500ms) in `TableRenderWidget.destroy()` mitigates some timing cases but does not fully resolve the issue. ([#19](https://github.com/saberzero1/motions/issues/19))
