...
- **Preview pane renders markdown** — file previews are rendered through `MarkdownRenderer.render()`, displaying headings, formatting, code blocks, images, and links. Positional previews (grep, headings, marks) include a line-number gutter with highlighted target line. Links inside the preview are non-interactive. The picker modal uses a fixed height (50vh) to prevent layout shifts when switching between files. Plain-string previews (commands, registers) remain as raw text.
