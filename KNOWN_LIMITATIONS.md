...
- The Escape handler in `embeddable-editor.ts` uses Obsidian's `Scope.register([], 'Escape', ...)` to handle Escape — this fires before vim's `eventObservers.keydown` observer, ensuring the handler sees vim state before vim clears it. The `isVimIdle()` helper checks all compound-command sub-states (`inputState.operator`, `surroundState`, `inputState.keyBuffer`, `expectLiteralNext`) in addition to `vim.mode`. When vim is idle (normal mode, no pending operation), Escape fires `onEscape()` to exit. Otherwise, the handler returns `true` to prevent parent Obsidian scopes from intercepting Escape, letting vim process the mode transition. The `onEscape()` callback is deferred via `requestAnimationFrame` so the Scope handler returns `true` (consuming the Escape event) while the editor's scope is still on the keymap stack — tearing down synchronously would pop the scope mid-handler, allowing Obsidian's keymap system to re-evaluate the same Escape against parent scopes (closing modals or switching the active leaf). The textarea-vim overlay additionally enables `isolateKeyEvents` which stops `keydown`/`keyup` propagation to prevent key events from leaking to parent modals. The embeddable editor uses a `_destroying` flag to prevent the blur event listener from double-popping the keymap scope when `destroy()` is already handling cleanup.
...
- ~~**Cannot leave table downwards when on last line of document**~~: Fixed. Pressing `j` at the last data row when the table occupies the final lines of the document now inserts a newline after the table and moves the cursor there, instead of silently staying stuck. ([#119](https://github.com/saberzero1/motions/issues/119))
- ~~**Unhandled keys swallowed in cell selection mode**~~: Fixed. The table-nav key handler previously called `preventDefault()`/`stopPropagation()` on ALL keys, including keys the handler did not process (e.g., leader key sequences). Unhandled keys now propagate to vim, enabling which-key popups and other vim key sequences during cell selection. ([#120](https://github.com/saberzero1/motions/issues/120))
- ~~**Which-key popups missing in cell selection mode**~~: Fixed. A `WhichKeyOverlay` instance is now attached during table-nav mode (cell selection), using the main editor's vim adapter. Previously, which-key only worked in cell-edit mode (inside a cell editor). ([#120](https://github.com/saberzero1/motions/issues/120))
- ~~**Picker focus stays on table widget**~~: Fixed. The `setActiveLeaf` override in `embeddable-editor.ts` now allows focus transfer when a modal (picker, command palette) is open, by checking for `.modal-container` in the DOM. ([#120](https://github.com/saberzero1/motions/issues/120))
- ~~**Table-nav key handler intercepts keys during modal/picker interaction**~~: Fixed. `handleTableNavKey` and `handleCellEditKey` now check for `.modal-container` and bail when a modal is open. Keys typed into picker inputs, command palette, and settings dialogs are no longer consumed by the table handler. ([#120](https://github.com/saberzero1/motions/issues/120))
- ~~**Modifier key combos consumed by vim during cell selection**~~: Fixed. `handleTableNavKey` now calls `e.stopPropagation()` for modifier key combos and feeds the event to Obsidian's keymap system via `app.keymap.onKeyEvent(e)`. This blocks vim from processing `<C-p>` as cursor-up while still triggering Obsidian's hotkey bindings. `Ctrl+P` (command palette), `Cmd+O` (file switcher), and other Obsidian shortcuts now work during cell selection. ([#120](https://github.com/saberzero1/motions/issues/120))
- ~~**Ex command dialog keys consumed by table-nav handler**~~: Fixed. `handleTableNavKey` now checks `adapter.state.dialog` and returns early when vim's ex command dialog is open. Keys like `h`, `j`, `k`, `l`, `a`, `i`, `c` typed into the `:` command prompt are no longer intercepted by the table handler. ([#120](https://github.com/saberzero1/motions/issues/120))
- ~~**Clicking outside table in embedded mode does not exit table-nav**~~: Fixed. A capture-phase `mousedown` listener on `activeDocument` detects clicks outside the table widget and calls `exitTable()` to reset the state machine. ([#121](https://github.com/saberzero1/motions/issues/121))
- ~~**Click-outside handler exits during modal interaction**~~: Fixed. The `mousedown` listener now checks for `.modal-container` before calling `exitTable()`, preventing modals from causing table-nav exit. ([#120](https://github.com/saberzero1/motions/issues/120))
- ~~**Header-only tables enter table-nav**~~: Fixed. Tables with only header + separator (no data rows) are now skipped by `checkEntry()`. ([#121](https://github.com/saberzero1/motions/issues/121))
- ~~**Stale table-nav state after document content replacement**~~: Fixed. The ViewPlugin's `update()` method now checks if the cursor is still in a table when `docChanged` fires during `table-nav` or `cell-edit` state, and exits gracefully if the table no longer exists. Previously, the controller could remain in `table-nav` state with stale references after `setupEditor()` replaced the document. ([#119](https://github.com/saberzero1/motions/issues/119), [#120](https://github.com/saberzero1/motions/issues/120))
- ~~**Cursor displacement when entering table-nav**~~: Fixed. `setActiveEditTableRange()` is now called before `this.view.dispatch()` in `enterTableNav()`, preventing `buildDecorations()` from creating a `Decoration.replace` during the dispatch that would snap the cursor to the table header row. ([#121](https://github.com/saberzero1/motions/issues/121))

**Cursor displacement guard**: In cursor-aware mode, a `transactionFilter` guards against Obsidian's Live Preview repositioning the cursor to the table header row during table creation or editing. When a `docChanged` transaction moves the cursor from a data row to the header row of the same table, the filter overrides the selection to keep the cursor at its mapped position. This guard only fires in cursor-aware mode — embedded mode uses its own `activeEditTableRange` mechanism.

**Table creation cursor jump (platform limitation)**: When creating a proper table by typing (e.g., `| test | test |\n| ----- |`) in cursor-aware mode in Live Preview, the cursor may jump to the header row. This occurs because Obsidian's Live Preview detects the table structure and repositions the cursor at the decoration rendering level — below the CM6 extension API. The `tableCursorGuard` transaction filter intercepts displacement visible to CM6 transactions, but some displacement occurs at a layer that does not produce CM6 transactions, ViewPlugin updates, or `requestAnimationFrame` callbacks. After closing and reopening the note, cursor movement through the table works normally. Workaround: use **Embedded** mode or **Source** mode for creating tables. ([#121](https://github.com/saberzero1/motions/issues/121))

**Wikilink cursor displacement in table cells**: Typing a wikilink (`[[`) inside a table cell in cursor-aware mode in Live Preview causes the cursor to jump past the closing `]]` after approximately 2 characters. This is an Obsidian platform limitation — Live Preview's internal link decoration system repositions the cursor during decoration rendering, below the CM6 extension API. The displacement does not go through CM6 transactions, ViewPlugin updates, or `requestAnimationFrame` callbacks, making it impossible to intercept from plugin code. This does not occur in **Source** mode or in **Embedded** table widget mode (which provides per-cell editors). Workaround: use `set tablewidget=embedded` for typing wikilinks in table cells. ([#121](https://github.com/saberzero1/motions/issues/121))
