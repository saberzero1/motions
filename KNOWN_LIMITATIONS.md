...
- **Extmark limitations**: `nvim_buf_set_extmark` supports a subset of Neovim's extmark options: `virt_text`, `virt_text_pos` (`"overlay"`, `"eol"`, `"inline"`), `hl_group`, `sign_text`, `priority`, `id`. Other options (`end_row`, `end_col`, `spell`, `conceal`, `undo_restore`, etc.) are not implemented. Buffer handle must be `0` (current buffer).
...
| `vim.api.nvim_*`                           | 60 real implementations (`nvim_create_user_command`, `nvim_del_user_command`, `nvim_create_autocmd`, `nvim_create_augroup`, `nvim_del_autocmd`, `nvim_del_augroup_by_name`, `nvim_clear_autocmds`, `nvim_create_namespace`, `nvim_set_hl`, `nvim_get_hl`, `nvim_set_keymap`, `nvim_del_keymap`, `nvim_get_keymap`, `nvim_buf_set_keymap`, `nvim_buf_del_keymap`, `nvim_buf_get_keymap`, `nvim_buf_get_lines`, `nvim_buf_set_lines`, `nvim_buf_set_text`, `nvim_buf_get_text`, `nvim_get_current_buf`, `nvim_get_current_win`, `nvim_get_current_tabpage`, `nvim_get_current_line`, `nvim_set_current_line`, `nvim_del_current_line`, `nvim_buf_get_name`, `nvim_buf_line_count`, `nvim_buf_is_valid`, `nvim_win_get_cursor`, `nvim_win_set_cursor`, `nvim_win_get_buf`, `nvim_list_wins`, `nvim_buf_get_mark`, `nvim_buf_set_mark`, `nvim_buf_del_mark`, `nvim_buf_get_var`, `nvim_buf_set_var`, `nvim_buf_get_option`, `nvim_buf_set_option`, `nvim_get_option`, `nvim_set_option`, `nvim_get_option_value`, `nvim_set_option_value`, `nvim_get_vvar`, `nvim_set_vvar`, `nvim_get_mode`, `nvim_strwidth`, `nvim_buf_set_extmark`, `nvim_buf_get_extmarks`, `nvim_buf_get_extmark_by_id`, `nvim_buf_del_extmark`, `nvim_buf_clear_namespace`, `nvim_command`, `nvim_feedkeys`, `nvim_replace_termcodes`, `nvim_echo`, `nvim_win_call`, `nvim_buf_call`, `nvim_win_get_config`); registered stubs return placeholders, and calls outside the registry error |
...
| `winnr()` / `tabpagenr()`             | Window/tab number — could map to workspace leaf index                                |
| `winwidth()` / `winheight()`          | Editor viewport dimensions                                                           |
| `confirm()`                           | Confirmation dialog — would require Obsidian modal UI integration                    |
| `virtcol(expr)`                       | Virtual column accounting for tabs — niche, `col()` suffices for most use cases      |
| `line2byte(lnum)` / `byte2line(byte)` | Byte offset conversion — niche                                                       |
...
- **`spike-mutable-array-destroy` fails on Windows CI**: The `eventObservers lifecycle` case has failed on Windows since before the #181 work and is unrelated to it — it fails identically on `f1b5626`, and no commit in that series touches the file. After clearing a mutable extension array and calling `workspace.updateOptions()`, the spike's `ViewPlugin` is destroyed but its `keydown` observer still fires once. It has not reproduced on Linux or macOS. The production shape is the same — the fork's key handling is a CM6 `eventObservers.keydown` on the vim `ViewPlugin`, which lives in the slot `disableVim()` clears — so this was checked rather than assumed: on Linux the editor's live keydown observer count goes 1 → 0 → 1 across repeated disable/enable cycles with no growth, and `test/specs/vim-toggle.e2e.ts` now asserts exactly that so Windows CI answers the same question for the real extension. In `@codemirror/view` 6.38.6 `ensureHandlers()` recomputes the handler map whenever the plugin set changes and removes the native listener once the last handler for an event type disappears, so a surviving observer means either the reconfigured state still lists the plugin or the key reached a different view. Windows answered it: `verdict=plugin-still-in-other-view attached=true observers=2 editors=2`. The keydown came from a second, DOM-attached editor that still had the plugin configured, while the active editor had already dropped it — Windows CI runs with two editors, Linux with one, which is why this never reproduced locally. CodeMirror is exonerated: the handler map matched the configuration in both views. What remains open is whether that second editor is an ordinary workspace leaf that `updateOptions()` should have reached, or an embedded editor that receives extensions through `StateEffect.appendConfig` and is therefore outside that mechanism by design. The spike now names every editor present and marks the one that fired; `test/specs/vim-toggle.e2e.ts` sums observers across every reconfigurable editor rather than sampling the active one, so the same blind spot cannot hide a real leak in the production vim extension.
