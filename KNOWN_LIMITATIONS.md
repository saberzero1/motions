...
- ~~**Modifier key combos consumed by vim during cell selection**~~: Fixed. `handleTableNavKey` now calls `e.stopPropagation()` for modifier key combos (`Ctrl`, `Alt`, `Meta`), preventing vim's `eventObservers.keydown` from processing them as cursor movement. `Ctrl+P` (command palette), `Cmd+O` (file switcher), and other Obsidian shortcuts now work during cell selection. ([#120](https://github.com/saberzero1/motions/issues/120))
