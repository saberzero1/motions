...
- **Sturdier preview error handling** — a synchronous throw from a picker source's `preview()` is caught as [[picker-api|the provider API]] documents, and superseded in-flight previews are rejected by generation instead of item id, so re-selecting the same item can no longer render a stale result
