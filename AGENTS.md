...
- **`npm run verify` runs all four static gates**: `lint`, `format:check` (Prettier, pinned as a devDependency), `lint:patterns` (`ast-grep scan`), and `lint:deadcode` (`knip`). All four are blocking in CI.
...
- **Known finding: the treesitter CM6 bridge is not wired up, and is not currently correct.** `createBridgeExtension` in `src/treesitter/bridge.ts` has no caller, so the per-view incremental-parsing `ViewPlugin` is never installed and `src/treesitter/tree-state.ts` is unused as a consequence. This matters more than it looks: `js-api.ts` has **no parse-on-demand fallback** — every function reads `getTreeForView`, which only the bridge populates — so `isTreeAvailable()` always returns false and the syntax-aware branches in `motions/headings.ts`, `text-objects/{code-block,delimiter,blockquote}.ts`, `snippets/context.ts` and `isInsideInlineNodeType` have never executed in production. They guard correctly and fall back to Lezer or regex. The Lua `vim.treesitter` API is unaffected; it has its own parser cache and parses on demand.

    Wiring it up was attempted and reverted (`9a03cc8`, reverted). Two blockers, both verified:
    1. `TreeSitterBridge`'s constructor calls `getOrCreateParser`, which **throws** until `loadLanguage` has resolved, and `initTreesitterRuntime` runs only from the Lua config loader. Registering the extension directly takes down every editor view for anyone not using Lua. Install it into a mutable slot after the grammar loads, then `workspace.updateOptions()`.
    2. `bridge.ts` `update()` applies only `update.transactions[0]`. A `ViewUpdate` can carry several, so the remaining changes never reach `tree.edit()`, the tree desyncs from the document, and the incremental re-parse returns nodes at wrong rows. This broke all 8 heading cases in `test/specs/navigation.e2e.ts` — the first time that code had ever run. Use the composed `update.changes` instead, and drop the unsound `!`.

    Fix (2) before attempting (1) again, and expect `navigation.e2e.ts` to be the acceptance test.
