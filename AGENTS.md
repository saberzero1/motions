...
- **nucleo-matcher-wasm fork**: The plugin uses a [fork of nucleo-matcher-wasm](https://github.com/saberzero1/nucleo-matcher-wasm) at `~/Repos/nucleo-matcher-wasm` for the optional WASM-based fuzzy matcher (used by the Telescope-style picker on desktop). The fork adds custom methods (`matchLiteralIndexedWithIndices`, `matchPatternIndexedWithIndices`) that return item array indices + scores + highlight character indices in a single WASM boundary crossing, and optimizes the binary for size (`opt-level = 'z'`, `lto = true`). The upstream is [idleberg/nucleo-matcher-wasm](https://github.com/idleberg/nucleo-matcher-wasm), which wraps the [nucleo-matcher](https://github.com/helix-editor/nucleo) crate from the Helix editor.
    - **IMPORTANT: dependency URL in `package.json`**: The `nucleo-matcher-wasm` dependency MUST point to `https://github.com/saberzero1/nucleo-matcher-wasm.git` (the remote URL) before committing. During local development, use `npm install ~/Repos/nucleo-matcher-wasm` for fast iteration, but **always switch back to the HTTPS URL before committing** — same rules as the codemirror-vim and fengari forks.
    - **Building the fork**: The fork requires Rust, `wasm-pack`, and `lld`. Use `nix develop` in the fork directory to get all dependencies. Run `pnpm run build` to produce `dist/` with CJS, ESM, and web targets. The `dist/` directory is committed to the repo (pre-built artifacts) so that `npm install` from a git URL works without requiring `wasm-pack` at install time.
    - **WASM binary size**: ~193 KB after `wasm-opt` optimization. Embedded in the plugin's `main.js` via esbuild's `binary` loader.
    - **What the fork adds over upstream**: `matchLiteralIndexedWithIndices` and `matchPatternIndexedWithIndices` methods that return `{ indices: Uint32Array, scores: Uint32Array, charIndices: Uint32Array[] }` — avoids O(n) string marshaling per match by returning array indices instead of matched strings, and includes highlight character positions in the same call.
    - **What the plugin uses**: The picker's `matcher-nucleo.ts` imports the web target (`nucleo-matcher-wasm/web`) for `initSync()` and the WASM binary (`nucleo-matcher-wasm/nucleo_wasm_bg.wasm`) embedded as a `Uint8Array`. On desktop, WASM is initialized synchronously via `initSync(wasmBytes)`. On mobile or on WASM failure, the plugin falls back to uFuzzy.
    - **License**: MPL-2.0 (compatible with the plugin's MIT license for bundling). Attribution in `ACKNOWLEDGEMENTS.md`.
