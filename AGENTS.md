...
- `test/specs/vim-builtin/` — Tier 1 tests (built-in CM Vim behavior). Use `testWithNeovim()` as primary format. Includes `new-commands.e2e.ts` (fork actions: `@:`, `&`, `ZZ`, `ZQ`, insert `<C-a>`/`<C-e>`/`<C-y>`), `link-nav-window-cycle.e2e.ts` (`<C-^>`, `<C-]>`, `<C-t>`, `<C-w>w`/`W`/`p`), `ex-move-copy-normal.e2e.ts` (`:m`, `:t`, `:normal`), `minor-motions-scroll.e2e.ts` (`gm`, `go`, `g8`, `gF`, `<C-g>`, `zs`/`ze`/`zH`/`zL`), `noop-commands.e2e.ts` (no-op crash guards).
...
- `wdio.conf.mts` — WDIO configuration. Includes a global `afterTest` hook that cleans up overlays (hint, easymotion, which-key, ex-suggest), dismisses notices, closes picker modals via Escape dispatch, closes generic modals via close-button click, sends double `<Esc>` to Vim, and verifies cleanup succeeded (force-removes any surviving elements on second pass).
