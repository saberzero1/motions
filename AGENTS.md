...
- **The treesitter CM6 bridge is installed asynchronously.** `createBridgeExtension` is pushed into a mutable extension slot by `enableTreesitterBridge()` in `main.ts` **only after `loadLanguage('markdown')` resolves**, then applied with `workspace.updateOptions()`. Do not register it directly: the bridge's `ViewPlugin` constructor calls `getOrCreateParser`, which throws until the grammar is loaded, and that would break every editor view. Until it resolves — and permanently if the WASM fails — `getTreeForView` returns null and every consumer takes the Lezer or regex path it already had. It had previously never been installed at all, so the syntax-aware branches in `motions/headings.ts`, `text-objects/{code-block,delimiter,blockquote}.ts`, `snippets/context.ts` and `isInsideInlineNodeType` had never executed in production.
