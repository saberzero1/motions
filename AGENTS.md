...
The desktop `GlobalKeyHandler` also owns the native File Explorer's fixed contextual `h`/`j`/`k`/`l` aliases when workspace navigation is enabled. It translates only unmodified, non-composing events outside editors, modals, inputs, and contenteditable controls into same-document arrow events, and suppresses those synthetic events from the physical-key observer before allowing Obsidian's native tree handler to consume them.
...
- **`npm run verify` runs all five static gates**: `typecheck` (`tsc -noEmit`), `lint`, `format:check` (Prettier, pinned as a devDependency), `lint:patterns` (`ast-grep scan`), and `lint:deadcode` (`knip`). All five are blocking in CI.
