...
src/
  ...
  vim/
    ...
    table-cell-motions.ts    # defineMotion overrides for h/j/k/l cross-cell navigation in native table cell editors — gated on tableWidgetMode=native (independent of enableTableNav), scheduleCrossing uses MessageChannel for deterministic task scheduling, signals animated cursor handoff via signalCellCrossing() before cell focus change
