...
src/
  ...
  vim/
    ...
    table-cell-motions.ts    # defineMotion overrides for h/j/k/l cross-cell navigation in native table cell editors — gated on tableWidgetMode=native (independent of enableTableNav), scheduleCrossing uses requestAnimationFrame to defer cross-cell focus changes until after Obsidian's table widget handlers complete (avoids macOS Electron race condition), signals animated cursor handoff via signalCellCrossing() before cell focus change
