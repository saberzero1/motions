...
- **Lua configuration** — `.obsidian.init.lua` with conditional logic, function keymaps, `vim.v` predefined variables (`count`, `count1`, `register`, `operator`, `searchforward`, constants), `{ expr = true }` expression mappings, `vim.fn.*`, `vim.api.*` (43 functions: buffer, cursor, marks, keymaps, options, commands, highlights, autocommands), `vim.tbl_*`, `vim.snippet.*`, `vim.json`, `vim.inspect`, `vim.regex` (ECMAScript RegExp), `vim.schedule`/`vim.defer_fn`/`vim.uv` timers, autocommands (19 events, mode events fire per-view across all editors), `vim.obsidian` namespace (including `vim.obsidian.im` for input method control), buffer-local keymaps, async file reading (`vim.ob.fs.read`), multi-file configs via `require()`, `collectgarbage()` support, `__gc` userdata finalization, `vim.o.operatorfunc` support for `g@`, `vim.treesitter` (full treesitter API backed by `web-tree-sitter` WASM — `get_parser`, `get_node`, `query.parse`, `Query:iter_captures`/`iter_matches`, LanguageTree, 31 TSNode methods, 8 built-in predicates, 4 directives), and Neovim-compatible syntax
