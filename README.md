...
**Third-party Lua compatibility:** API counts are not proof that a Neovim plugin runs. mini.surround and mini.splitjoin remain audit-blocked; their integration phases were cancelled pending a follow-up plan. The built-in surround feature above is separate. Existing mini.comment tests cover specific operations but fetch a moving `main` branch, a known reproducibility risk rather than an immutable compatibility pin. flash.nvim is blocked by unavailable LuaJIT FFI. See [API status](NEOVIM_API_STATUS.md) for the audited blockers.
