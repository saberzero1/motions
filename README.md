...
- **Line numbers** — configurable line number gutter with absolute, relative, and hybrid modes matching Neovim's `number`/`relativenumber` semantics. Suppresses Obsidian's native line numbers when active. Configure via settings, `set number`/`set relativenumber` in vimrc, or `vim.opt.number`/`vim.opt.relativenumber` in Lua
