...
- `test/flaky-inventory.md`: three CI failures observed during this work. `keeps source-rendered frontmatter fully navigable` reproduced on Windows with diagnostics byte-identical to the inventoried macOS case (`rows [7,6,6,6,6,6]`, `mode: "source"`, `metadataContainers: 1`), which confirms the prediction already recorded against it — the `setPropertiesSource()` gate is reporting the wrong mode and the fault is not platform-specific. It moves from environment-suspected to product-side, and is added to the main inventory table, which had been missing it. The other two — `paints an embedded note the same way` (macOS) and `leaves the stock behaviour when the setting is disabled` (Windows). Both are recorded as `Unknown` with the observed values; a passing re-run is not a classification.
