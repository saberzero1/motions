import { to_luastring, constant_types, thread_status } from './defs.js';
...
const save = function (ls: LexState, c: number): void {
    let b = ls.buff!;
...
const luaX_token2str = function (ls: LexState, token: number): Uint8Array {
    if (token < FIRST_RESERVED) {
        ...
        else /* names, strings, and numerals */ return s;
    }
};

const currIsNewline = function (ls: LexState): boolean {
    return (
        ls.current === 10 /* ('\n').charCodeAt(0) */ || ls.current === 13
    ); /* ('\r').charCodeAt(0) */
};

const next = function (ls: LexState): void {
    ls.current = ls.z!.zgetc();
};

const save_and_next = function (ls: LexState): void {
    save(ls, ls.current);
    next(ls);
};
...
const luaX_newstring = function (
    ls: LexState,
    str: ArrayLike<number>,
): TString {
    let L = ls.L!;
 ...
 ** \n, \r, \n\r, or \r\n)
 */
const inclinenumber = function (ls: LexState): void {
    let old = ls.current;
...
const check_next1 = function (ls: LexState, c: number): boolean {
    if (ls.current === c) {
        next(ls);
        return true;
    }
 ...
 ** saves it
 */
const check_next2 = function (ls: LexState, set: string): boolean {
    if (ls.current === set.charCodeAt(0) || ls.current === set.charCodeAt(1)) {
        save_and_next(ls);
        return true;
    }
...
const read_numeral = function (ls: LexState, seminfo: SemInfo): number {
    let expo = 'Ee';
    let first = ls.current;
    lua_assert(lisdigit(ls.current));
    save_and_next(ls);
    if (first === 48 /* ('0').charCodeAt(0) */ && check_next2(ls, 'xX'))
        /* hexadecimal? */
        expo = 'Pp';
    ...
    for (;;) {
        ...
        else if (ls.current === 46 /* ('.').charCodeAt(0) */) save_and_next(ls);
        else break;
    }

    // save(ls, 0);

    let obj = new lobject.TValue(LUA_TNIL, null);
    ...
    if (obj.ttisinteger()) {
        ...
        seminfo.r = obj.fltvalue();
        return TK_FLT;
    }
};

const txtToken = function (ls: LexState, token: number): Uint8Array {
    switch (token) {
        case TK_NAME:
        case TK_STRING:
        case TK_FLT:
...
const lexerror = function (ls: LexState, msg: Uint8Array, token: number): void {
    msg = ldebug.luaG_addinfo(ls.L!, msg, ls.source, ls.linenumber);
    if (token)
        lobject.luaO_pushfstring(
            ls.L!,
            to_luastring('%s near %s'),
            msg,
            txtToken(ls, token),
        );
...
const luaX_syntaxerror = function (ls: LexState, msg: Uint8Array): void {
 ...
 */
const skip_sep = function (ls: LexState): number {
    let count = 0;
    let s = ls.current;
    lua_assert(
        s === 91 /* ('[').charCodeAt(0) */ ||
...
const read_long_string = function (
    ls: LexState,
    seminfo: SemInfo | null,
    sep: number,
): void {
    ...
    if (seminfo)
        seminfo.ts = luaX_newstring(
            ls,
            ls.buff!.buffer!.subarray(2 + sep, ls.buff!.n - (2 + sep)),
        );
};

const esccheck = function (ls: LexState, c: boolean, msg: Uint8Array): void {
    if (!c) {
        if (ls.current !== EOZ)
...
const gethexa = function (ls: LexState): number {
    save_and_next(ls);
    esccheck(
        ls,
        lisxdigit(ls.current),
        to_luastring('hexadecimal digit expected', true),
    );
    return lobject.luaO_hexavalue(ls.current);
};

const readhexaesc = function (ls: LexState): number {
    let r = gethexa(ls);
    r = (r << 4) + gethexa(ls);
...
const readutf8desc = function (ls: LexState): number {
    let i = 4; /* chars to be removed: '\', 'u', '{', and first digit */
    save_and_next(ls); /* skip 'u' */
    esccheck(
        ls,
        ls.current === 123 /* ('{').charCodeAt(0) */,
    ...
    while (lisxdigit(ls.current)) {
        ...
        esccheck(
            ls,
            r <= 0x10ffff,
            to_luastring('UTF-8 value too large', true),
        );
        save_and_next(ls);
    }
    esccheck(
        ls,
        ls.current === 125 /* ('}').charCodeAt(0) */,
        to_luastring("missing '}'", true),
    );
    next(ls); /* skip '}' */
...
const utf8esc = function (ls: LexState): void {
...
const readdecesc = function (ls: LexState): number {
...
const read_string = function (
    ...
    while (ls.current !== del) {
        switch (ls.current) {
            ...
            case 13 /* ('\r').charCodeAt(0) */:
                lexerror(
                    ls,
                    to_luastring('unfinished string', true),
                    TK_STRING,
                );
                break;
            case 92 /* ('\\').charCodeAt(0) */: {
                ...
                let will: 'read_save' | 'no_save' | 'only_save';
                let c = 0;
                switch (ls.current as number) {
                    case 97 /* ('a').charCodeAt(0) */:
                        c = 7; /* \a isn't valid JS */
                        will = 'read_save';
                        break;
                    ...
                    default: {
                        esccheck(
                            ls,
                            lisdigit(ls.current),
                            to_luastring('invalid escape sequence', true),
                        );
    ...
    seminfo.ts = luaX_newstring(
        ls,
        ls.buff!.buffer!.subarray(1, ls.buff!.n - 1),
    );
};

const token_to_index: Record<string, number> = Object.create(null) as Record<
    string,
    number
>;
luaX_tokens.forEach((e, i) => (token_to_index[luaS_hash(e)] = i));

const isreserved = function (w: TString): boolean {
    let kidx = token_to_index[luaS_hashlongstr(w)];
    return kidx !== void 0 && kidx <= 22;
};

const llex = function (ls: LexState, seminfo: SemInfo): number {
    ...
    for (;;) {
        ...
        switch (ls.current) {
            ...
            case 45 /* ('-').charCodeAt(0) */: {
                /* '-' or '--' (comment) */
                next(ls);
                if ((ls.current as number) !== 45 /* ('-').charCodeAt(0) */)
                    return 45; /* ('-').charCodeAt(0) */
                /* else is a comment */
                next(ls);
                if ((ls.current as number) === 91 /* ('[').charCodeAt(0) */) {
                    /* long comment? */
                    let sep = skip_sep(ls);
                    luaZ_resetbuffer(
                        ls.buff!,
                ...
                /* else short comment */
                while (!currIsNewline(ls) && (ls.current as number) !== EOZ)
            ...
            case 91 /* ('[').charCodeAt(0) */: {
                ...
                if (sep >= 0) {
                    ...
                    lexerror(
                        ls,
                        to_luastring('invalid long string delimiter', true),
                        TK_STRING,
                    );
            ...
            case 60 /* ('<').charCodeAt(0) */: {
                ...
                else if (check_next1(ls, 60 /* ('<').charCodeAt(0) */))
                    return TK_SHL;
            ...
            case 62 /* ('>').charCodeAt(0) */: {
                ...
                else if (check_next1(ls, 62 /* ('>').charCodeAt(0) */))
                    return TK_SHR;
                else return 62; /* ('>').charCodeAt(0) */
            }
            case 47 /* ('/').charCodeAt(0) */: {
                next(ls);
                if (check_next1(ls, 47 /* ('/').charCodeAt(0) */))
                    return TK_IDIV;
            ...
            case 58 /* (':').charCodeAt(0) */: {
                next(ls);
                if (check_next1(ls, 58 /* (':').charCodeAt(0) */))
                    return TK_DBCOLON;
            ...
            case 49 /* ('1').charCodeAt(0) */:
            case 50 /* ('2').charCodeAt(0) */:
            case 51 /* ('3').charCodeAt(0) */:
            case 52 /* ('4').charCodeAt(0) */:
            case 53 /* ('5').charCodeAt(0) */:
            case 54 /* ('6').charCodeAt(0) */:
            case 55 /* ('7').charCodeAt(0) */:
            case 56 /* ('8').charCodeAt(0) */:
            case 57 /* ('9').charCodeAt(0) */: {
...
const luaX_next = function (ls: LexState): void {
...
const luaX_lookahead = function (ls: LexState): number {
