...
const { luaS_bless, luaS_new, luaS_newliteral } = require('./lstring.js');
...
const index2addr = function (L, idx) {
    ...
    if (idx > 0) {
        ...
        if (ci.func.ttislcf())
            ...
            return idx <= ci.func.value.nupvalues
                ? ci.func.value.upvalue[idx - 1]
...
const lua_rawget = function (L, idx) {
    ...
    lobject.setobj2s(
        L,
        L.top - 1,
        ltable.luaH_get(L, t.value, L.stack[L.top - 1]),
    );
    return L.stack[L.top - 1].ttnov();
};
