 ...
 */
const checktab = function (L: lua_State, arg: number, what: number): void {
    if (lua_type(L, arg) !== LUA_TTABLE) {
        /* is it not a table? */
        let n = 1;
        if (
            lua_getmetatable(L, arg) /* must have metatable */ &&
            (!(what & TAB_R) ||
                checkfield(L, to_luastring('__index', true), ++n)) &&
            (!(what & TAB_W) ||
                checkfield(L, to_luastring('__newindex', true), ++n)) &&
...
const aux_getn = function (L: lua_State, n: number, w: number): number {
    checktab(L, n, w | TAB_L);
    return luaL_len(L, n);
};

const addfield = function (L: lua_State, b: luaL_Buffer, i: number): void {
    lua_geti(L, 1, i);
    if (!lua_isstring(L, -1))
        luaL_error(
            L,
...
const tinsert = function (L: lua_State): number {
    ...
    switch (lua_gettop(L)) {
        ...
        default: {
            return luaL_error(
                L,
                to_luastring("wrong number of arguments to 'insert'"),
            );
        }
    }
...
const tremove = function (L: lua_State): number {
 ...
 */
const tmove = function (L: lua_State): number {
    ...
    if (e >= f) {
        ...
        if (
            t > e ||
            t <= f ||
            (tt !== 1 && lua_compare(L, 1, tt, LUA_OPEQ) !== 1)
        ) {
            for (let i = 0; i < n; i++) {
                lua_geti(L, 1, f + i);
                lua_seti(L, tt, t + i);
...
const tconcat = function (L: lua_State): number {
    let last = aux_getn(L, 1, TAB_R);
    let sep = luaL_optlstring(L, 2, '') ?? to_luastring('', true);
    let lsep = sep.length;
...
const pack = function (L: lua_State): number {
    let n = lua_gettop(L); /* number of elements to pack */
    lua_createtable(L, n, 1); /* create result table */
    lua_insert(L, 1); /* put it at index 1 */
    for (let i = n; i >= 1; i--) /* assign elements */ lua_seti(L, 1, i);
    lua_pushinteger(L, n);
    lua_setfield(L, 1, to_luastring('n')); /* t.n = number of elements */
    return 1; /* return table */
};

const unpack = function (L: lua_State): number {
...
const l_randomizePivot = function (): number {
    return Math.floor(Math.random() * 0x100000000);
};

const RANLIMIT = 100;

const set2 = function (L: lua_State, i: number, j: number): void {
    lua_seti(L, 1, i);
    lua_seti(L, 1, j);
};

const sort_comp = function (L: lua_State, a: number, b: number): boolean {
    if (lua_isnil(L, 2))
        /* no function? */
        return lua_compare(L, a, b, LUA_OPLT) === 1; /* a < b */
    else {
        /* function */
...
const partition = function (L: lua_State, lo: number, up: number): number {
    ...
    for (;;) {
        ...
        while ((lua_geti(L, 1, ++i), sort_comp(L, -1, -2))) {
            if (i == up - 1)
                ...
                luaL_error(
                    L,
                    to_luastring('invalid order function for sorting'),
                );
...
const choosePivot = function (lo: number, up: number, rnd: number): number {
...
const auxsort = function (
    ...
    while (lo < up) {
        ...
        if (up - lo == 1) /* only 2 elements? */ return; /* already sorted */
        let p; /* Pivot index */
        ...
        if (up - lo == 2) /* only 3 elements? */ return; /* already sorted */
...
const sort = function (L: lua_State): number {
...
const tab_funcs: Record<string, LuaCFunction> = {
    concat: tconcat,
    insert: tinsert,
    move: tmove,
    pack: pack,
...
const luaopen_table = function (L: lua_State): number {
