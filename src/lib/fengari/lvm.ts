...
import { INDEXK, ISK, LFIELDS_PER_FLUSH, OpCodesI } from './lopcodes.js';
import {
    LUA_MAXINTEGER,
    LUA_MININTEGER,
    lua_numbertointeger,
} from './luaconf.js';
import { lua_assert, luai_nummod } from './llimits.js';
import * as lobject from './lobject.js';
import * as lfunc from './lfunc.js';
import * as lstate from './lstate.js';
import { luaS_bless, luaS_eqlngstr, luaS_hashlongstr } from './lstring.js';
import * as ldo from './ldo.js';
import * as ltm from './ltm.js';
import * as ltable from './ltable.js';
import * as ldebug from './ldebug.js';
import type { TValue, LClosure } from './lobject.js';
import type { lua_State, CallInfo } from './lstate.js';
 ...
 */
const luaV_finishOp = function (L: lua_State): void {
    let ci = L.ci!;
    let base = ci.l_base;
    let code = ci.l_code as unknown as Instruction[];
    let inst = code[ci.l_savedpc - 1]!; /* interrupted instruction */
    let op = inst.opcode;
    let stack = L.stack!;
    ...
    switch (op /* finish its execution */) {
        ...
        case OP_EQ: {
            ...
            lua_assert(code[ci.l_savedpc]!.opcode === OP_JMP);
        ...
        case OP_TFORCALL: {
            lua_assert(code[ci.l_savedpc]!.opcode === OP_TFORLOOP);
...
const RA = function (L: lua_State, base: number, i: Instruction): number {
    return base + i.A;
};

const RB = function (L: lua_State, base: number, i: Instruction): number {
    return base + i.B;
};
...
const luaV_execute = function (L: lua_State): void {
    let ci = L.ci!;

    ci.callstatus |= lstate.CIST_FRESH;
    newframe: for (;;) {
        lua_assert(ci === L.ci);
        let cl = ci.func!.clLvalue();
        let k = cl.p!.k;
        let base = ci.l_base;
        let code = ci.l_code as unknown as Instruction[];
        let i = code[ci.l_savedpc++]!;
        let stack = L.stack!;
        ...
        switch (opcode) {
            ...
            case OP_LOADKX: {
                lua_assert(code[ci.l_savedpc]!.opcode === OP_EXTRAARG);
                let konst = k[code[ci.l_savedpc++]!.Ax]!;
                lobject.setobj2s(L, ra, konst);
                break;
            }
            case OP_LOADBOOL: {
                stack[ra]!.setbvalue(i.B !== 0);
            ...
            case OP_LOADNIL: {
                for (let j = 0; j <= i.B; j++) stack[ra + j]!.setnilvalue();
                break;
            }
            case OP_GETUPVAL: {
                let b = i.B;
            ...
            case OP_NEWTABLE: {
                stack[ra]!.sethvalue(ltable.luaH_new(L));
                break;
            }
            case OP_SELF: {
                let rb = RB(L, base, i);
                ...
                luaV_gettable(L, stack[rb]!, rc, ra);
                break;
            }
            case OP_ADD: {
                ...
                if (op1.ttisinteger() && op2.ttisinteger()) {
                    stack[ra]!.setivalue(op1.value + op2.value);
                } else if (
                    (numberop1 = tonumber(op1)) !== false &&
                    (numberop2 = tonumber(op2)) !== false
                ) {
                    stack[ra]!.setfltvalue(numberop1 + numberop2);
                } else {
                    ltm.luaT_trybinTM(L, op1, op2, stack[ra]!, ltm.TMS.TM_ADD);
                }
                break;
            }
            case OP_SUB: {
                ...
                if (op1.ttisinteger() && op2.ttisinteger()) {
                    ...
                    ltm.luaT_trybinTM(L, op1, op2, stack[ra]!, ltm.TMS.TM_SUB);
                }
                break;
            }
            case OP_MUL: {
                ...
                if (op1.ttisinteger() && op2.ttisinteger()) {
                    stack[ra]!.setivalue(luaV_imul(op1.value, op2.value));
                } else if (
                    (numberop1 = tonumber(op1)) !== false &&
                    (numberop2 = tonumber(op2)) !== false
                ) {
                    ...
                    ltm.luaT_trybinTM(L, op1, op2, stack[ra]!, ltm.TMS.TM_MUL);
                }
                break;
            }
            case OP_MOD: {
                ...
                if (op1.ttisinteger() && op2.ttisinteger()) {
                    stack[ra]!.setivalue(luaV_mod(L, op1.value, op2.value));
                } else if (
                    (numberop1 = tonumber(op1)) !== false &&
                    (numberop2 = tonumber(op2)) !== false
                ) {
                    stack[ra]!.setfltvalue(
                        luai_nummod(L, numberop1, numberop2),
                    );
                } else {
                    ltm.luaT_trybinTM(L, op1, op2, stack[ra]!, ltm.TMS.TM_MOD);
                }
                break;
            }
            case OP_POW: {
                ...
                if (
                    ...
                    stack[ra]!.setfltvalue(Math.pow(numberop1, numberop2));
                } else {
                    ltm.luaT_trybinTM(L, op1, op2, stack[ra]!, ltm.TMS.TM_POW);
                }
                break;
            }
            case OP_DIV: {
                ...
                if (
                    ...
                    ltm.luaT_trybinTM(L, op1, op2, stack[ra]!, ltm.TMS.TM_DIV);
                }
                break;
            }
            case OP_IDIV: {
                ...
                if (op1.ttisinteger() && op2.ttisinteger()) {
                    stack[ra]!.setivalue(luaV_div(L, op1.value, op2.value));
                } else if (
                    (numberop1 = tonumber(op1)) !== false &&
                    (numberop2 = tonumber(op2)) !== false
                ) {
                    stack[ra]!.setfltvalue(Math.floor(numberop1 / numberop2));
                } else {
                    ltm.luaT_trybinTM(L, op1, op2, stack[ra]!, ltm.TMS.TM_IDIV);
                }
                break;
            }
            case OP_BAND: {
                ...
                if (
                    (numberop1 = tointeger(op1)) !== false &&
                    (numberop2 = tointeger(op2)) !== false
                ) {
                    stack[ra]!.setivalue(numberop1 & numberop2);
                } else {
                    ltm.luaT_trybinTM(L, op1, op2, stack[ra]!, ltm.TMS.TM_BAND);
                }
                break;
            }
            case OP_BOR: {
                ...
                if (
                    ...
                    ltm.luaT_trybinTM(L, op1, op2, stack[ra]!, ltm.TMS.TM_BOR);
                }
                break;
            }
            case OP_BXOR: {
                ...
                if (
                    ...
                    ltm.luaT_trybinTM(L, op1, op2, stack[ra]!, ltm.TMS.TM_BXOR);
                }
                break;
            }
            case OP_SHL: {
                ...
                if (
                    ...
                    stack[ra]!.setivalue(luaV_shiftl(numberop1, numberop2));
                } else {
                    ltm.luaT_trybinTM(L, op1, op2, stack[ra]!, ltm.TMS.TM_SHL);
                }
                break;
            }
            case OP_SHR: {
                ...
                if (
                    ...
                    ltm.luaT_trybinTM(L, op1, op2, stack[ra]!, ltm.TMS.TM_SHR);
                }
                break;
            }
            case OP_UNM: {
                let op = stack[RB(L, base, i)]!;
                let numberop;

                if (op.ttisinteger()) {
                    stack[ra]!.setivalue(-op.value);
                } else if ((numberop = tonumber(op)) !== false) {
                    stack[ra]!.setfltvalue(-numberop);
                } else {
                    ltm.luaT_trybinTM(L, op, op, stack[ra]!, ltm.TMS.TM_UNM);
                }
                break;
            }
            case OP_BNOT: {
                ...
                if (op.ttisinteger()) {
                    ...
                    ltm.luaT_trybinTM(L, op, op, stack[ra]!, ltm.TMS.TM_BNOT);
                }
                break;
            }
            case OP_NOT: {
                let op = stack[RB(L, base, i)]!;
                stack[ra]!.setbvalue(op.l_isfalse());
                break;
            }
            case OP_LEN: {
                luaV_objlen(L, stack[ra]!, stack[RB(L, base, i)]!);
                break;
            }
            case OP_CONCAT: {
                let b = i.B;
            ...
            case OP_EQ: {
                if (
                    luaV_equalobj(L, RKB(L, base, k, i), RKC(L, base, k, i)) !==
                    i.A
                )
                    ci.l_savedpc++;
                else donextjump(L, ci);
                break;
            }
            case OP_LT: {
                if (
                    luaV_lessthan(L, RKB(L, base, k, i), RKC(L, base, k, i)) !==
                    i.A
                )
                    ci.l_savedpc++;
                else donextjump(L, ci);
            ...
            case OP_TEST: {
                if (i.C ? stack[ra]!.l_isfalse() : !stack[ra]!.l_isfalse())
                    ci.l_savedpc++;
                else donextjump(L, ci);
                break;
            }
            case OP_TESTSET: {
                ...
                let rb = stack[rbIdx]!;
                if (i.C ? rb.l_isfalse() : !rb.l_isfalse()) ci.l_savedpc++;
                else {
            ...
            case OP_TAILCALL: {
                ...
                if (ldo.luaD_precall(L, ra, LUA_MULTRET)) {
                    ...
                    let nclosure = nfunc.clLvalue();
                    let lim = nci.l_base + nclosure.p!.numparams;
                    ...
                    lua_assert(
                        L.top ===
                            oci.l_base +
                                stack[ofuncOff]!.clLvalue().p!.maxstacksize,
                    );
            ...
            case OP_RETURN: {
                ...
                lua_assert((ci.callstatus & lstate.CIST_LUA) !== 0);
                lua_assert(
                    (ci.l_code as unknown as Instruction[])[ci.l_savedpc - 1]!
                        .opcode === OP_CALL,
                );
                continue newframe;
            }
            case OP_FORLOOP: {
                if (stack[ra]!.ttisinteger()) {
                    /* integer loop? */
                    let step = stack[ra + 2]!.nvalue();
                    let idx = stack[ra]!.nvalue() + step;
                    let limit = stack[ra + 1]!.nvalue();
                    ...
                    if (0 < step ? idx <= limit : limit <= idx) {
                        ci.l_savedpc += i.sBx;
                        stack[ra]!.chgfltvalue(
                            idx,
                        ); /* update internal index... */
                        stack[ra + 3]!.setfltvalue(idx);
                    }
                }
                break;
            }
            case OP_FORPREP: {
                let init = stack[ra]!;
                let plimit = stack[ra + 1]!;
                let pstep = stack[ra + 2]!;
                let forlim;

                if (
                    init.ttisinteger() &&
                    pstep.ttisinteger() &&
                    (forlim = forlimit(plimit, pstep.value))
                ) {
                    /* all values are integer */
                    let limitInfo = forlim;
                    let initv = limitInfo.stopnow ? 0 : init.value;
                    plimit.value = limitInfo.ilimit;
                    ...
                    if ((nlimit = tonumber(plimit)) === false)
                        ldebug.luaG_runerror(
                            L,
                            to_luastring("'for' limit must be a number", true),
                        );
                    stack[ra + 1]!.setfltvalue(nlimit as number);
                    if ((nstep = tonumber(pstep)) === false)
                        ldebug.luaG_runerror(
                            L,
                            to_luastring("'for' step must be a number", true),
                        );
                    stack[ra + 2]!.setfltvalue(nstep as number);
                    ...
                    stack[ra]!.setfltvalue(
                        (ninit as number) - (nstep as number),
                    );
                }
            ...
            case OP_TFORCALL: {
                ...
                i = (ci.l_code as unknown as Instruction[])[ci.l_savedpc++]!;
                ra = RA(L, base, i);
                lua_assert(i.opcode === OP_TFORLOOP);
                if (!stack[ra + 1]!.ttisnil()) {
                    /* continue loop? */
                    lobject.setobjs2s(
                        L,
                        ra,
            ...
            case OP_SETLIST: {
                ...
                if (c === 0) {
                    lua_assert(
                        (ci.l_code as unknown as Instruction[])[ci.l_savedpc]!
                            .opcode === OP_EXTRAARG,
                    );
                    c = (ci.l_code as unknown as Instruction[])[ci.l_savedpc++]!
                        .Ax;
                }

                let h = stack[ra]!.hvalue();
                let last = (c - 1) * LFIELDS_PER_FLUSH + n;

                for (; n > 0; n--) {
                    ltable.luaH_setint(h, last--, stack[ra + n]!);
                }
                ldo.adjust_top(
                    L,
                    ci.top,
...
const donextjump = function (L: lua_State, ci: CallInfo): void {
    dojump(L, ci, (ci.l_code as unknown as Instruction[])[ci.l_savedpc]!, 1);
};

const luaV_lessthan = function (L: lua_State, l: TValue, r: TValue): number {
    if (l.ttisnumber() && r.ttisnumber()) return LTnum(l, r) ? 1 : 0;
...
const luaV_lessequal = function (L: lua_State, l: TValue, r: TValue): number {
    let res: boolean | null | number;

    if (l.ttisnumber() && r.ttisnumber()) return LEnum(l, r) ? 1 : 0;
    else if (l.ttisstring() && r.ttisstring())
        return l_strcmp(l.tsvalue(), r.tsvalue()) <= 0 ? 1 : 0;
    else {
...
const luaV_equalobj = function (
    ...
    if (t1.ttype() !== t2.ttype()) {
        ...
        if (t1.ttnov() !== t2.ttnov() || t1.ttnov() !== LUA_TNUMBER)
            ...
            return t1.nvalue() === t2.nvalue() ? 1 : 0;
        }
    }
    ...
    switch (t1.ttype()) {
        case LUA_TNIL:
            return 1;
        case LUA_TBOOLEAN:
            return t1.bvalue() == t2.bvalue() ? 1 : 0; // Might be 1 or true
        case LUA_TLIGHTUSERDATA:
            return t1.pvalue() === t2.pvalue() ? 1 : 0;
        case LUA_TNUMINT:
        case LUA_TNUMFLT:
            return t1.nvalue() === t2.nvalue() ? 1 : 0;
        case LUA_TLCF:
            return t1.fvalue() === t2.fvalue() ? 1 : 0;
        case LUA_TSHRSTR:
        case LUA_TLNGSTR: {
        ...
        case LUA_TTABLE:
            ...
            tm = ltm.fasttm(
                L,
                t1.ttistable() ? t1.hvalue().metatable : t1.uvalue().metatable,
                ltm.TMS.TM_EQ,
            );
            if (tm === null)
                tm = ltm.fasttm(
    ...
    if (tm === null) /* no TM? */ return 0;

    let tv = new lobject.TValue(LUA_TNIL, null); /* doesn't use the stack */
    ltm.luaT_callTM(L, tm, t1, t2, tv, 1);
    return tv.l_isfalse() ? 0 : 1;
};

const luaV_rawequalobj = function (t1: TValue, t2: TValue): number {
    return luaV_equalobj(null, t1, t2);
};

const forlimit = function (
    obj: TValue,
    step: number,
): false | { stopnow: boolean; ilimit: number } {
    let stopnow = false;
 ...
 */
const luaV_tointeger = function (obj: TValue, mode: number): number | false {
    if (obj.ttisfloat()) {
        let n = obj.value;
        let f = Math.floor(n);
...
const tointeger = function (o: TValue): number | false {
    return o.ttisinteger() ? o.ivalue() : luaV_tointeger(o, 0);
};

const tonumber = function (o: TValue): number | false {
    if (o.ttnov() === LUA_TNUMBER) return o.nvalue();
 ...
 */
const LTnum = function (l: TValue, r: TValue): boolean {
    return l.nvalue() < r.nvalue();
};

/*
 ** Return 'l <= r', for numbers.
 */
const LEnum = function (l: TValue, r: TValue): boolean {
    return l.nvalue() <= r.nvalue();
};
 ...
 */
const l_strcmp = function (ls: TString, rs: TString): number {
 ...
 */
const luaV_objlen = function (L: lua_State, ra: TValue, rb: TValue): void {
    let tm: TValue | null;
    switch (rb.ttype()) {
        case LUA_TTABLE: {
            let h = rb.hvalue();
        ...
        default: {
            ...
            if (tm.ttisnil())
                ldebug.luaG_typeerror(
                    L,
                    rb,
                    to_luastring('get length of', true),
                );
            break;
        }
    }
...
const luaV_imul = function (a: number, b: number): number {
    return a * b;
};

const luaV_div = function (L: lua_State | null, m: number, n: number): number {
    if (n === 0)
...
const luaV_mod = function (L: lua_State | null, m: number, n: number): number {
    if (n === 0)
...
const luaV_shiftl = function (x: number, y: number): number {
    if (y < 0) {
        /* shift right? */
        if (y <= -NBITS) return 0;
        else return x >>> -y;
...
const cvt2str = function (o: TValue): boolean {
    return o.ttisnumber();
};

const cvt2num = function (o: TValue): boolean {
    return o.ttisstring();
};

const tostring = function (L: lua_State, i: number): boolean {
    let o = L.stack![i]!;
...
const isemptystr = function (o: TValue): boolean {
    return o.ttisstring() && o.vslen() === 0;
};

/* copy strings in stack from top - n up to top - 1 to buffer */
const copy2buff = function (
    L: lua_State,
    top: number,
    n: number,
    buff: Uint8Array,
 ...
 */
const luaV_concat = function (L: lua_State, total: number): void {
    lua_assert(total >= 2);
    do {
        let top = L.top;
        ...
        if (
            !(L.stack![top - 2]!.ttisstring() || cvt2str(L.stack![top - 2]!)) ||
            !tostring(L, top - 1)
        ) {
            ltm.luaT_trybinTM(
                L,
...
const luaV_gettable = function (
    ...
    ldebug.luaG_runerror(
        L,
        to_luastring("'__index' chain too long; possible loop", true),
    );
};
...
const settable = function (
    ...
    for (let loop = 0; loop < MAXTAGLOOP; loop++) {
        ...
        if (t.ttistable()) {
            ...
            if (
                !slot.ttisnil() ||
                (tm = ltm.fasttm(L, h.metatable, ltm.TMS.TM_NEWINDEX)) === null
            ) {
    ...
    ldebug.luaG_runerror(
        L,
        to_luastring("'__newindex' chain too long; possible loop", true),
    );
};
