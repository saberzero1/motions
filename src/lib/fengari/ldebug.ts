...
import type { TValue, CClosure, LClosure } from './lobject.js';
import type { CallInfo, lua_State } from './lstate.js';

const { LUA_TBOOLEAN, LUA_TNIL, LUA_TTABLE } = constant_types;
const { LUA_ERRRUN, LUA_YIELD } = thread_status;

type NameInfo = {
    name: Uint8Array | null;
    funcname: Uint8Array | null;
};

type CallInfoWithExtra = CallInfo & { extra: number };

type HookFunction = (L: lua_State, ar: lua_Debug) => void;
...
const currentpc = function (ci: CallInfo): number {
    lua_assert((ci.callstatus & lstate.CIST_LUA) !== 0);
    return ci.l_savedpc - 1;
};

const currentline = function (ci: CallInfo): number {
    const func = ci.func!;
    const cl = func.clLvalue();
    const proto = cl.p!;
    return proto.lineinfo.length !== 0 ? proto.lineinfo[currentpc(ci)]! : -1;
};
 ...
 */
const swapextra = function (L: lua_State): void {
    if (L.status === LUA_YIELD) {
        let ci = L.ci! as CallInfoWithExtra; /* get function that yielded */
...
const lua_sethook = function (
    L: lua_State,
    func: HookFunction | null,
    mask: number,
    count: number,
): void {
    if (func === null || mask === 0) {
...
const lua_gethook = function (L: lua_State): HookFunction | null {
    return L.hook;
};

const lua_gethookmask = function (L: lua_State): number {
    return L.hookmask;
};

const lua_gethookcount = function (L: lua_State): number {
    return L.basehookcount;
};

const lua_getstack = function (
    L: lua_State,
    level: number,
    ar: lua_Debug,
): number {
    let ci: CallInfo | null;
    let status: number;
...
const upvalname = function (p: Proto, uv: number): Uint8Array {
...
const findvararg = function (
    ci: CallInfo,
    n: number,
): { pos: number; name: Uint8Array } | null {
    const func = ci.func!;
    const cl = func.clLvalue();
    let nparams = cl.p!.numparams;
...
const findlocal = function (
    ...
    if (ci.callstatus & lstate.CIST_LUA) {
        if (n < 0) /* access to vararg values? */ return findvararg(ci, -n);
        else {
            base = ci.l_base;
            const func = ci.func!;
            const cl = func.clLvalue();
            name = lfunc.luaF_getlocalname(cl.p!, n, currentpc(ci)) ?? null;
        }
...
const lua_getlocal = function (
    L: lua_State,
    ar: lua_Debug | null,
    n: number,
): Uint8Array | null {
    let name: Uint8Array | null;
    swapextra(L);
    if (ar === null) {
        ...
        if (!L.stack![L.top - 1]!.ttisLclosure())
            ...
            name =
                lfunc.luaF_getlocalname(
                    L.stack![L.top - 1]!.clLvalue().p!,
                    n,
                    0,
                ) ?? null;
    } else {
        /* active function; get information through 'ar' */
        let local = findlocal(L, ar.i_ci as CallInfo, n);
        if (local) {
            name = local.name;
...
const funcinfo = function (
    ...
    if (cl === null || cl instanceof lobject.CClosure) {
        ...
        ar.what =
            ar.linedefined === 0
                ? to_luastring('main', true)
                : to_luastring('Lua', true);
    }

    ar.short_src = lobject.luaO_chunkid(ar.source, LUA_IDSIZE);
};

const collectvalidlines = function (
    L: lua_State,
    f: LClosure | CClosure | null,
): void {
...
const getfuncname = function (
    ...
    /* calling function is a known Lua function? */
        ...
        ci.previous!.callstatus & lstate.CIST_LUA
    )
        return funcnamefromcode(L, ci.previous!);
    else return null; /* no way to find a name */
};

const auxgetinfo = function (
    L: lua_State,
    what: Uint8Array,
    ar: lua_Debug,
    f: LClosure | CClosure | null,
    ...
    for (; what.length > 0; what = what.subarray(1)) {
        switch (what[0]) {
            ...
            case 108 /* ('l').charCodeAt(0) */: {
                ar.currentline =
                    ci && ci.callstatus & lstate.CIST_LUA
                        ? currentline(ci)
                        : -1;
                break;
            }
            case 117 /* ('u').charCodeAt(0) */: {
                ...
                if (f === null || f instanceof lobject.CClosure) {
                    ar.isvararg = true as unknown as number;
                    ar.nparams = 0;
                } else {
                    ar.isvararg = f.p!.is_vararg as unknown as number;
...
const lua_getinfo = function (
    ...
    if (what[0] === 62 /* ('>').charCodeAt(0) */) {
        ...
        ci = ar.i_ci as CallInfo;
        func = ci.func!;
        lua_assert(ci.func!.ttisfunction());
    }

    cl = func.ttisLclosure()
        ? func.clLvalue()
        : func.ttisCclosure()
          ? func.clCvalue()
          : null;
...
const kname = function (p: Proto, pc: number, c: number): NameInfo {
    let r: NameInfo = {
        name: null,
        funcname: null,
    };
    ...
    if (lopcodes.ISK(c)) {
        ...
        if (
            what?.funcname &&
            what.funcname[0] === 99 /* ('c').charCodeAt(0) */
        ) {
...
const filterpc = function (pc: number, jmptarget: number): number {
    if (pc < jmptarget)
 ...
 */
const findsetreg = function (p: Proto, lastpc: number, reg: number): number {
    ...
    for (let pc = 0; pc < lastpc; pc++) {
        let i = p.code[pc] as unknown as Instruction;
        let a = i.A;
        switch (i.opcode) {
            case OCi.OP_LOADNIL: {
                let b = i.B;
...
const getobjname = function (
    p: Proto,
    lastpc: number,
    reg: number,
): NameInfo | null {
    let r: NameInfo = {
        name: lfunc.luaF_getlocalname(p, reg + 1, lastpc) ?? null,
        funcname: null,
    };
    ...
    if (pc !== -1) {
        ...
        switch (i.opcode) {
            ...
            case OCi.OP_GETTABLE: {
                ...
                let vn =
                    i.opcode === OCi.OP_GETTABLE
                        ? (lfunc.luaF_getlocalname(p, t + 1, pc) ?? null)
                        : upvalname(p, t);
                r.name = kname(p, pc, k).name;
                r.funcname =
                    vn && luastring_eq(vn, llex.LUA_ENV)
                        ? to_luastring('global', true)
                        : to_luastring('field', true);
                return r;
            }
            ...
            case OCi.OP_LOADKX: {
                let b =
                    i.opcode === OCi.OP_LOADK
                        ? i.Bx
                        : (p.code[pc + 1] as unknown as Instruction).Ax;
                if (p.k[b]!.ttisstring()) {
                    r.name = p.k[b].svalue();
 ...
 */
const funcnamefromcode = function (
    L: lua_State,
    ci: CallInfo,
): NameInfo | null {
    let r: NameInfo = {
    ...
    let p = ci.func!.clLvalue().p!; /* calling function */
    let pc = currentpc(ci); /* calling instruction index */
    let i = p.code[pc] as unknown as Instruction; /* calling instruction */
    let OCi = lopcodes.OpCodesI;
 ...
 ** upvalues.)
 */
const getupvalname = function (
    L: lua_State,
    ci: CallInfo,
    o: TValue,
): NameInfo | null {
    let c = ci.func!.clLvalue();
...
const varinfo = function (L: lua_State, o: TValue): Uint8Array {
    let ci = L.ci!;
    let kind: NameInfo | null = null;
    if (ci.callstatus & lstate.CIST_LUA) {
...
const luaG_typeerror = function (
    ...
    luaG_runerror(
        L,
        to_luastring('attempt to %s a %s value%s', true),
        op,
        t,
        varinfo(L, o),
    );
...
const luaG_concaterror = function (L: lua_State, p1: TValue, p2: TValue): void {
 ...
 */
const luaG_opinterror = function (
    L: lua_State,
    p1: TValue,
    p2: TValue,
    msg: Uint8Array,
...
const luaG_ordererror = function (L: lua_State, p1: TValue, p2: TValue): void {
    let t1 = ltm.luaT_objtypename(L, p1);
    let t2 = ltm.luaT_objtypename(L, p2);
    if (luastring_eq(t1, t2))
        luaG_runerror(
            L,
            to_luastring('attempt to compare two %s values', true),
            t1,
        );
    else
        luaG_runerror(
            L,
            to_luastring('attempt to compare %s with %s', true),
            t1,
            t2,
        );
};
...
const luaG_addinfo = function (
    ...
    let buff: Uint8Array;
    if (src) buff = lobject.luaO_chunkid(src.getstr(), LUA_IDSIZE);
    else buff = to_luastring('?', true);

    return lobject.luaO_pushfstring(
        L,
        to_luastring('%s:%d: %s', true),
        buff,
        line,
        msg,
    );
...
const luaG_runerror = function (
    ...
    if (ci.callstatus & lstate.CIST_LUA)
        ...
        luaG_addinfo(L, msg, ci.func!.clLvalue().p!.source, currentline(ci));
    luaG_errormsg(L);
};

const luaG_errormsg = function (L: lua_State): void {
    if (L.errfunc !== 0) {
 ...
 ** Error when both values are convertible to numbers, but not to integers
 */
const luaG_tointerror = function (L: lua_State, p1: TValue, p2: TValue): void {
    let temp = lvm.tointeger(p1);
    if (temp === false) p2 = p1;
    luaG_runerror(
        L,
        to_luastring('number%s has no integer representation', true),
        varinfo(L, p2),
    );
};

const luaG_traceexec = function (L: lua_State): void {
    let ci = L.ci!;
    let mask = L.hookmask;
