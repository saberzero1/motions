...
const luaSprintf = function (format: string, value: unknown): string {
    const percentIndex = format.indexOf('%');
    if (percentIndex === -1) return format;

    if (format[percentIndex + 1] === '%') {
        const nextPercent = format.indexOf('%', percentIndex + 2);
        if (nextPercent === -1)
            return (
                format.slice(0, percentIndex) +
                '%' +
                format.slice(percentIndex + 2)
            );
    }
    ...
    while (i < format.length && LUA_FORMAT_FLAGS.includes(format[i]!)) {
        if (!flags.includes(format[i]!)) flags += format[i]!;
        i++;
    }
    ...
    if (i > widthStart) width = parseInt(format.slice(widthStart, i), 10);
    ...
    if (format[i]! === '.') {
        ...
        precision =
            precisionStart === i
                ? 0
                : parseInt(format.slice(precisionStart, i), 10);
    }

    while (i < format.length && 'hlLjztI'.includes(format[i]!)) i++;
    ...
    const applyWidth = function (text: string, padChar = ' '): string {
    ...
    const applyNumericWidth = function (
        sign: string,
        prefixText: string,
        digits: string,
        padChar: string,
        precisionSpecified: boolean,
    ): string {
        let base = sign + prefixText + digits;
        if (width === null || base.length >= width) return base;
    ...
    const formatInteger = function (
        ...
        if (flags.includes('#')) {
            if (base === 8) {
                if (digits[0] !== '0') prefixText = '0';
                if (num === 0 && precision === 0) prefixText = '0';
    ...
    const trimTrailingZeros = function (text: string): string {
    ...
    const formatFloat = function (n: unknown, specifier: string): string {
        let num = Number(n);
        let sign = '';
        if (num < 0) {
            sign = '-';
        ...
        switch (specifier) {
            ...
            case 'E':
                ...
                if (
                    ...
                    formatted =
                        formatted.slice(0, expPos) +
                        '.' +
                        formatted.slice(expPos);
                }
                break;
            case 'f':
        ...
        if (
            (specifier === 'E' || specifier === 'G') &&
            formatted.indexOf('e') !== -1
        )
...
const strlen = function (s: LuaString): number {
    let len = luastring_indexOf(s, 0);
    return len > -1 ? len : s.length;
};

/* translate a relative string position: negative means back from end */
const posrelat = function (pos: number, len: number): number {
    if (pos >= 0) return pos;
    else if (0 - pos > len) return 0;
    else return len + pos + 1;
};

const str_sub = function (L: lua_State): number {
...
const str_len = function (L: lua_State): number {
    lua_pushinteger(L, luaL_checkstring(L, 1).length);
    return 1;
};

const str_char = function (L: lua_State): number {
...
const writer = function (
    ...
    const B = data as luaL_Buffer;
    luaL_addlstring(B, b, size);
    return 0;
};

const str_dump = function (L: lua_State): number {
    let b = new luaL_Buffer();
    let strip = lua_toboolean(L, 2) ? 1 : 0;
...
const num2straux = function (x: number): LuaString {
    /* if 'inf' or 'NaN', format it like '%g' */
    if (Object.is(x, Infinity)) return to_luastring('inf');
    else if (Object.is(x, -Infinity)) return to_luastring('-inf');
    else if (Number.isNaN(x)) return to_luastring('nan');
    else if (x === 0) {
        /* can be -0... */
...
const lua_number2strx = function (
    L: lua_State,
    fmt: LuaString,
    x: number,
): LuaString {
    ...
    if (fmt[SIZELENMOD] === 65 /* 'A'.charCodeAt(0) */) {
        for (let i = 0; i < buff.length; i++) {
            let c = buff[i]!;
            if (c >= 97) /* toupper */ buff[i] = c & 0xdf;
        }
    } else if (fmt[SIZELENMOD] !== 97 /* 'a'.charCodeAt(0) */)
        luaL_error(
            L,
            to_luastring("modifiers for format '%%a'/'%%A' not implemented"),
        );
    return buff;
};
...
const isalpha = (e: number): boolean =>
    (97 <= e && e <= 122) || (65 <= e && e <= 90);
const isdigit = (e: number): boolean => 48 <= e && e <= 57;
const iscntrl = (e: number): boolean => (0x00 <= e && e <= 0x1f) || e === 0x7f;
const isgraph = (e: number): boolean => 33 <= e && e <= 126;
const islower = (e: number): boolean => 97 <= e && e <= 122;
const isupper = (e: number): boolean => 65 <= e && e <= 90;
const isalnum = (e: number): boolean =>
    (97 <= e && e <= 122) || (65 <= e && e <= 90) || (48 <= e && e <= 57);
const ispunct = (e: number): boolean => isgraph(e) && !isalnum(e);
const isspace = (e: number): boolean => e === 32 || (e >= 9 && e <= 13);
const isxdigit = (e: number): boolean =>
    (48 <= e && e <= 57) || (65 <= e && e <= 70) || (97 <= e && e <= 102);

const addquoted = function (b: luaL_Buffer, s: LuaString, len: number): void {
    ...
    while (len--) {
        if (
            ...
            if (isdigit(s[i + 1]!))
                buff = ('000' + buff).slice(
                    -3,
                ); /* pad to 3 digits with leading '0's */
 ...
 */
const checkdp = function (buff: LuaString): void {
    if (luastring_indexOf(buff, 46 /* ('.').charCodeAt(0) */) < 0) {
        ...
        if (ppoint)
            buff[ppoint] =
                46; /* ('.').charCodeAt(0) */ /* change it to a dot */
    }
};

const addliteral = function (L: lua_State, b: luaL_Buffer, arg: number): void {
    switch (lua_type(L, arg)) {
        ...
        case LUA_TNUMBER: {
            ...
            if (!lua_isinteger(L, arg)) {
                ...
                buff = lua_number2strx(
                    L,
                    to_luastring(`%${LUA_INTEGER_FRMLEN}a`),
                    n,
                );
...
const scanformat = function (
    L: lua_State,
    strfrmt: LuaString,
    i: number,
    form: LuaFormatBuffer,
): number {
    let p = i;
    ...
    if (isdigit(strfrmt[p]!))
        luaL_error(
            L,
            to_luastring('invalid format (width or precision too long)'),
        );
 ...
 */
const addlenmod = function (form: LuaFormatBuffer, lenmod: LuaString): void {
    let l = form.length;
...
const str_format = function (L: lua_State): number {
    ...
    while (i < strfrmt.length) {
        if (strfrmt[i]! !== L_ESC) {
            ...
            /* format item */
            let form: LuaFormatBuffer = []; /* to store the format ('%...') */
            ...
            switch (String.fromCharCode(strfrmt[i++]!)) {
                ...
                case 'X': {
                    ...
                    const formString = String.fromCharCode(...form);
                    luaL_addstring(b, to_luastring(luaSprintf(formString, n)));
                    break;
                }
                case 'a':
                case 'A': {
                ...
                default: {
                    ...
                    return luaL_error(
                        L,
                        to_luastring("invalid option '%%%c' to 'format'"),
                        strfrmt[i - 1]!,
                    );
                }
            }
...
const getnumlimit = function (
    ...
    if (sz > MAXINTSIZE || sz <= 0)
        luaL_error(
            h.L,
            to_luastring('integral size (%d) out of limits [1,%d]'),
            sz,
            MAXINTSIZE,
        );
    return sz;
...
const getoption = function (
    ...
    switch (r.opt) {
        ...
        case 102 /*'f'*/:
            r.size = 4;
            r.opt = Kfloat;
            return r; // sizeof(float): 4
        case 100 /*'d'*/:
            r.size = 8;
            r.opt = Kfloat;
            return r; // sizeof(double): 8
        case 110 /*'n'*/:
            r.size = 8;
            r.opt = Kfloat;
            return r; // sizeof(lua_Number): 8
        case 105 /*'i'*/:
            r.size = getnumlimit(h, fmt, 4);
            r.opt = Kint;
            return r; // sizeof(int): 4
        case 73 /*'I'*/:
            r.size = getnumlimit(h, fmt, 4);
            r.opt = Kuint;
            return r;
        ...
        case 99 /*'c'*/: {
            ...
            if (r.size === -1)
                luaL_error(
                    h.L,
                    to_luastring("missing size for format option 'c'"),
                );
            r.opt = Kchar;
            return r;
        }
...
const getdetails = function (
    ...
    totalsize: number,
    fmt: { s: LuaString; off: number },
): { opt: number; size: number; ntoalign: number } {
    let r = {
        opt: NaN,
        size: NaN,
        ntoalign: NaN,
    ...
    if (r.opt === Kpaddalign) {
        ...
        if (fmt.off >= fmt.s.length || fmt.s[fmt.off]! === 0)
            luaL_argerror(
                h.L,
                1,
                to_luastring("invalid next option for option 'X'"),
            );
        else {
            const nextOpt = getoption(h, fmt);
            align = nextOpt.size;
            const nextOptValue = nextOpt.opt;
            if (nextOptValue === Kchar || align === 0)
                luaL_argerror(
                    h.L,
                    1,
                    to_luastring("invalid next option for option 'X'"),
    ...
    if (align <= 1 || r.opt === Kchar) /* need no alignment? */ r.ntoalign = 0;
    else {
        if (align > h.maxalign)
        ...
        if ((align & (align - 1)) !== 0)
            ...
            luaL_argerror(
                h.L,
                1,
                to_luastring('format asks for alignment not power of 2'),
            );
...
const str_pack = function (L: lua_State): number {
    let b = new luaL_Buffer();
    let h = new Header(L);
    let fmt = {
    ...
    while (fmt.off < fmt.s.length) {
        ...
        switch (opt) {
            ...
            case Kuint: {
                ...
                else luaL_argcheck(L, n >= 0, arg, 'unsigned overflow');
            ...
            case Kstring: {
                ...
                packint(b, len, h.islittle, size, false); /* pack length */
            ...
            case Kzstr: {
                ...
                luaL_argcheck(
                    L,
                    luastring_indexOf(s, 0) < 0,
                    arg,
                    'strings contains zeros',
                );
                luaL_addlstring(b, s, len);
...
const str_reverse = function (L: lua_State): number {
    let s = luaL_checkstring(L, 1);
    let l = s.length;
...
const str_lower = function (L: lua_State): number {
    let s = luaL_checkstring(L, 1);
    let l = s.length;
    let r = new Uint8Array(l);
    for (let i = 0; i < l; i++) {
...
const str_upper = function (L: lua_State): number {
    let s = luaL_checkstring(L, 1);
    let l = s.length;
    let r = new Uint8Array(l);
    for (let i = 0; i < l; i++) {
...
const str_rep = function (L: lua_State): number {
    let s = luaL_checkstring(L, 1);
    let l = s.length;
...
const str_byte = function (L: lua_State): number {
    let s = luaL_checkstring(L, 1);
    let l = s.length;
    ...
    if (pose - posi >= Number.MAX_SAFE_INTEGER)
        /* arithmetic overflow? */
        return luaL_error(L, to_luastring('string slice too long'));
...
const str_packsize = function (L: lua_State): number {
    let h = new Header(L);
    let fmt = {
        ...
        switch (opt) {
            ...
            case Kzstr /* zero-terminated string */:
                return luaL_argerror(
                    L,
                    1,
                    to_luastring('variable-length format'),
                );
            default:
                break;
        }
...
const unpacknum = function (
    ...
    for (let i = 0; i < size; i++) dv.setUint8(i, b[i]!);

    if (size == 4) return dv.getFloat32(0, islittle);
    else return dv.getFloat64(0, islittle);
};

const str_unpack = function (L: lua_State): number {
    let h = new Header(L);
    let fmt = {
        s: luaL_checkstring(L, 1),
        off: 0,
...
class MatchState {
    ...
    matchdepth: number;
    level: number;
    capture: Array<{ init: number; len: number }>;

    constructor(L: lua_State) {
        this.src = new Uint8Array(0); /* unmodified source string */
        this.src_init = 0; /* init of source string */
        this.src_end = 0; /* end ('\0') of source string */
        this.p = new Uint8Array(0); /* unmodified pattern string */
        this.p_end = 0; /* end ('\0') of pattern */
        this.L = L;
        this.matchdepth = NaN; /* control for recursive depth */
        this.level =
            NaN; /* total number of captures (finished or unfinished) */
        this.capture = [];
    }
}

const check_capture = function (ms: MatchState, l: number): number {
    l = l - 49; /* '1'.charCodeAt(0) */
    if (l < 0 || l >= ms.level || ms.capture[l]!.len === CAP_UNFINISHED)
        return luaL_error(
            ms.L,
            to_luastring('invalid capture index %%%d'),
            l + 1,
        );
    return l;
};

const capture_to_close = function (ms: MatchState): number {
    let level = ms.level;
...
const classend = function (ms: MatchState, p: number): number {
    switch (ms.p[p++]!) {
        case L_ESC: {
            if (p === ms.p_end)
                luaL_error(
                    ms.L,
                    to_luastring("malformed pattern (ends with '%%')"),
                );
            return p + 1;
        }
        case 91 /* '['.charCodeAt(0) */: {
            ...
            do {
                ...
                if (p === ms.p_end)
                    luaL_error(
                        ms.L,
                        to_luastring("malformed pattern (missing ']')"),
                    );
...
const match_class = function (c: number, cl: number): boolean {
    switch (cl) {
...
const matchbracketclass = function (
    ms: MatchState,
    c: number,
    p: number,
    ec: number,
...
const matchbalance = function (
    ...
    if (p >= ms.p_end - 1)
        luaL_error(
            ms.L,
            to_luastring("malformed pattern (missing arguments to '%%b'"),
        );
    if (ms.src[s]! !== ms.p[p]!) return null;
    else {
        let b = ms.p[p];
        let e = ms.p[p + 1]!;
        let cont = 1;
...
const start_capture = function (
    ...
    if (level >= LUA_MAXCAPTURES)
        luaL_error(ms.L, to_luastring('too many captures'));
    ms.capture[level] = ms.capture[level] ?? { init: 0, len: 0 };
...
const match_capture = function (
    ...
    if (
        ms.src_end - s >= len &&
        array_cmp(ms.src, ms.capture[l]!.init, ms.src, s, len)
    )
        return s + len;
    else return null;
};

const match = function (ms: MatchState, s: number, p: number): number | null {
        ...
        if (p !== ms.p_end) {
            ...
            switch (gotodefault ? void 0 : ms.p[p]!) {
                case 40 /* '('.charCodeAt(0) */: {
                    /* start capture */
                    if (ms.p[p + 1]! === 41 /* ')'.charCodeAt(0) */) {
                        /* position capture? */
                        const res = start_capture(ms, s, p + 2, CAP_POSITION);
                        if (res === null) return null;
                        s = res;
                    } else {
                        const res = start_capture(ms, s, p + 1, CAP_UNFINISHED);
                        if (res === null) return null;
                        s = res;
                    }
                    break;
                ...
                case 36 /* '$'.charCodeAt(0) */: {
                    ...
                    if (ms.src.length - s !== 0)
                        return null; /* check end of string */
                    break;
                }
                case L_ESC: {
                    ...
                    switch (ms.p[p + 1]!) {
                        ...
                        case 102 /* 'f'.charCodeAt(0) */: {
                            ...
                            let previous =
                                s === ms.src_init ? 0 : ms.src[s - 1]!;
                            if (
                                !matchbracketclass(ms, previous, p, ep - 1) &&
                                matchbracketclass(
                                    ms,
                                    s === ms.src_end ? 0 : ms.src[s]!,
                                    p,
                                    ep - 1,
                                )
                            ) {
                        ...
                        case 57: {
                            /* capture results (%0-%9)? */
                            const res = match_capture(ms, s, ms.p[p + 1]!);
                            if (res !== null) {
                                s = res;
                                p += 2;
                                gotoinit = true;
                    ...
                    if (!singlematch(ms, s, p, ep)) {
                        ...
                        switch (ms.p[ep]! /* handle optional suffix */) {
                            ...
                            case 43 /* '+'.charCodeAt(0) */ /* 1 or more repetitions */:
                                ...
                                {
                                    const res = max_expand(ms, s, p, ep);
                                    if (res === null) return null;
                                    s = res;
                                }
                                break;
                            ...
                            case 45 /* '-'.charCodeAt(0) */ /* 0 or more repetitions (minimum) */:
                                {
                                    const res = min_expand(ms, s, p, ep);
                                    if (res === null) return null;
                                    s = res;
                                }
                                break;
...
const push_onecapture = function (
    ms: MatchState,
    i: number,
    s: number,
    e: number,
    ...
    if (i >= ms.level) {
        ...
        if (l === CAP_UNFINISHED)
            luaL_error(ms.L, to_luastring('unfinished capture'));
        if (l === CAP_POSITION)
            lua_pushinteger(ms.L, ms.capture[i]!.init - ms.src_init + 1);
        else lua_pushlstring(ms.L, ms.src.subarray(ms.capture[i]!.init), l);
    }
};

const push_captures = function (
    ms: MatchState,
    s: number | null,
    e: number,
): number {
...
const nospecials = function (p: LuaString, l: number): boolean {
    for (let i = 0; i < l; i++) {
...
const reprepstate = function (ms: MatchState): void {
    ms.level = 0;
...
const str_find_aux = function (L: lua_State, find: number): number {
    let s = luaL_checkstring(L, 1);
...
const str_find = function (L: lua_State): number {
    return str_find_aux(L, 1);
};

const str_match = function (L: lua_State): number {
...
class GMatchState {
    ...
    constructor(L: lua_State) {
        ...
        this.ms = new MatchState(L); /* match state */
    }
}

const gmatch_aux = function (L: lua_State): number {
    let gm = lua_touserdata(L, lua_upvalueindex(3)) as GMatchState;
    gm.ms.L = L;
...
const str_gmatch = function (L: lua_State): number {
    let s = luaL_checkstring(L, 1);
    let p = luaL_checkstring(L, 2);
    let ls = s.length;
    let lp = p.length;
    lua_settop(L, 2); /* keep them on closure to avoid being collected */
    let gm = new GMatchState(L);
...
const add_value = function (
    ...
    if (!lua_toboolean(L, -1)) {
        ...
        luaL_error(
            L,
            to_luastring('invalid replacement value (a %s)'),
            luaL_typename(L, -1),
        );
    luaL_addvalue(b); /* add result to accumulator */
};

const str_gsub = function (L: lua_State): number {
    ...
    while (n < max_s) {
        ...
        reprepstate(ms);
        if (
            (e = match(ms, srcIndex, patternIndex)) !== null &&
            e !== lastmatch
        ) {
            /* match? */
            n++;
            add_value(ms, b, srcIndex, e, tr); /* add replacement to buffer */
            srcIndex = lastmatch = e;
        } else if (srcIndex < ms.src_end)
            /* otherwise, skip one character */
            luaL_addchar(b, ms.src[srcIndex++]!);
        else break; /* end of subject */
        if (anchor) break;
    }
    luaL_addlstring(
        b,
        ms.src.subarray(srcIndex, ms.src_end),
        ms.src_end - srcIndex,
    );
    luaL_pushresult(b);
...
const strlib: Record<string, LuaCFunction> = {
    byte: str_byte,
    char: str_char,
    dump: str_dump,
    find: str_find,
...
const createmetatable = function (L: lua_State): void {
    ...
    lua_setfield(
        L,
        -2,
        to_luastring('__index', true),
    ); /* metatable.__index = string */
    lua_pop(L, 1); /* pop metatable */
};

const luaopen_string = function (L: lua_State): number {
