...
type Utf8DecodeResult = { code: number; pos: number };

const MAXUNICODE = 0x10ffff;

const iscont = function (p: number): boolean {
    const c = p & 0xc0;
    return c === 0x80;
};

/* translate a relative string position: negative means back from end */
const u_posrelat = function (pos: number, len: number): number {
    if (pos >= 0) return pos;
 ...
 */
const limits: number[] = [0xff, 0x7f, 0x7ff, 0xffff];
const utf8_decode = function (
    s: Uint8Array,
    pos: number,
): Utf8DecodeResult | null {
    let c = s[pos]!;
 ...
 ** that interval
 */
const utflen = function (L: lua_State): number {
    let n = 0;
...
const pushutfchar = function (L: lua_State, arg: number): void {
 ...
 */
const utfchar = function (L: lua_State): number {
    let n = lua_gettop(L); /* number of arguments */
    if (n === 1) /* optimize common case of single char */ pushutfchar(L, 1);
    else {
 ...
 */
const byteoffset = function (L: lua_State): number {
    let s = luaL_checkstring(L, 1);
    ...
    if (n === 0) {
        ...
        if (iscont(s[posi]!))
            luaL_error(
                L,
                to_luastring('initial position is a continuation byte'),
            );
    ...
    else /* no such character */ lua_pushnil(L);
 ...
 */
const codepoint = function (L: lua_State): number {
    let s = luaL_checkstring(L, 1);
...
const iter_aux = function (L: lua_State): number {
    let s = luaL_checkstring(L, 1);
    let len = s.length;
    let n = lua_tointeger(L, 2) - 1;

    if (n < 0) /* first iteration? */ n = 0; /* start from here */
    else if (n < len) {
        n++; /* skip current byte */
        while (n < len && iscont(s[n]!)) n++; /* and its continuations */
    }
...
const iter_codes = function (L: lua_State): number {
...
const funcs: Record<string, LuaCFunction> = {
    char: utfchar,
    codepoint: codepoint,
    codes: iter_codes,
    len: utflen,
...
const luaopen_utf8 = function (L: lua_State): number {
    luaL_newlib(L, funcs);
