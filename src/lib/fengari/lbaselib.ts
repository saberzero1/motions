...
type LuaStringOutput = Uint8Array | string;

let lua_writestring: (s: Uint8Array) => void;
let lua_writeline: () => void;
if (typeof TextDecoder === 'function') {
    let buff = '';
    let decoder = new TextDecoder('utf-8');
    lua_writestring = function (s: Uint8Array): void {
...
const luaB_print = function (L: lua_State): number {
    ...
    for (let i = 1; i <= n; i++) {
        ...
        if (s === null)
            return luaL_error(
                L,
                to_luastring("'tostring' must return a string to 'print'"),
            );
...
const luaB_tostring = function (L: lua_State): number {
    luaL_checkany(L, 1);
    luaL_tolstring(L, 1);
...
const luaB_getmetatable = function (L: lua_State): number {
    luaL_checkany(L, 1);
...
const luaB_setmetatable = function (L: lua_State): number {
    let t = lua_type(L, 2);
    ...
    if (luaL_getmetafield(L, 1, to_luastring('__metatable', true)) !== LUA_TNIL)
        return luaL_error(
            L,
            to_luastring('cannot change a protected metatable'),
        );
    lua_settop(L, 2);
...
const luaB_rawequal = function (L: lua_State): number {
    luaL_checkany(L, 1);
    luaL_checkany(L, 2);
    lua_pushboolean(L, lua_rawequal(L, 1, 2) !== 0);
    return 1;
};

const luaB_rawlen = function (L: lua_State): number {
    let t = lua_type(L, 1);
    luaL_argcheck(
        L,
...
const luaB_rawget = function (L: lua_State): number {
    luaL_checktype(L, 1, LUA_TTABLE);
    luaL_checkany(L, 2);
    lua_settop(L, 2);
    lua_rawget(L, 1);
...
const luaB_rawset = function (L: lua_State): number {
    luaL_checktype(L, 1, LUA_TTABLE);
    luaL_checkany(L, 2);
    luaL_checkany(L, 3);
    lua_settop(L, 3);
...
const luaB_collectgarbage = function (L: lua_State): number {
...
const luaB_type = function (L: lua_State): number {
    let t = lua_type(L, 1);
...
const luaB_next = function (L: lua_State): number {
    luaL_checktype(L, 1, LUA_TTABLE);
...
const luaB_pairs = function (L: lua_State): number {
    return pairsmeta(L, to_luastring('__pairs', true), false, luaB_next);
};

/*
 ** Traversal function for 'ipairs'
 */
const ipairsaux = function (L: lua_State): number {
 ...
 */
const luaB_ipairs = function (L: lua_State): number {
    // Lua 5.2
...
const b_str2int = function (s: Uint8Array, base: number): number | null {
    let jsString: string;
    try {
...
const luaB_tonumber = function (L: lua_State): number {
    if (lua_type(L, 2) <= 0) {
        /* standard conversion? */
        luaL_checkany(L, 1);
...
const luaB_error = function (L: lua_State): number {
...
const luaB_assert = function (L: lua_State): number {
...
const luaB_select = function (L: lua_State): number {
    let n = lua_gettop(L);
    if (
        lua_type(L, 1) === LUA_TSTRING &&
        lua_tostring(L, 1)![0] === 35 /* '#'.charCodeAt(0) */
    ) {
...
const luaB_pcall = function (L: lua_State): number {
    luaL_checkany(L, 1);
    lua_pushboolean(L, true); /* first result if no errors */
    lua_insert(L, 1); /* put it in place */
    let status = lua_pcallk(
        L,
        lua_gettop(L) - 2,
        LUA_MULTRET,
        0,
 ...
 */
const luaB_xpcall = function (L: lua_State): number {
    let n = lua_gettop(L);
 ...
 */
const generic_reader = function (L: lua_State, ud: unknown): Uint8Array | null {
    const data = ud as { string: Uint8Array | null } | null;
...
const luaB_load = function (L: lua_State): number {
    let s = lua_tostring(L, 1);
    ...
    if (s !== null) {
        /* loading a string? */
        let chunkname = luaL_optstring(L, 2, s) ?? to_jsstring(s);
...
const luaB_loadfile = function (L: lua_State): number {
...
const dofilecont = function (L: lua_State, _d1: number, _d2: number): number {
    return lua_gettop(L) - 1;
};

const luaB_dofile = function (L: lua_State): number {
    let fname = luaL_optstring(L, 1, null);
    lua_settop(L, 1);
...
const base_funcs: Record<string, LuaCFunction> = {
    assert: luaB_assert,
...
const luaopen_base = function (L: lua_State): number {
