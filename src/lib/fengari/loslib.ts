...
import { isDesktop, requireModule } from './platform.js';
...
const setfield = function (L: lua_State, key: string, value: number): void {
    lua_pushinteger(L, value);
    lua_setfield(L, -2, to_luastring(key, true));
};

const setallfields = function (
    L: lua_State,
    time: Date,
    utc: boolean = false,
): void {
...
const push_pad_2 = function (b: luaL_Buffer, n: number, pad: number): void {
    if (n < 10) luaL_addchar(b, pad);
    luaL_addstring(b, to_luastring(String(n)));
};

const strftime = function (
    L: lua_State,
    b: luaL_Buffer,
    s: Uint8Array,
    date: Date,
        ...
        if (s[i]! !== 37 /* % */) {
            ...
            switch (s[i]!) {
                ...
                case 115 /* s */:
                    luaL_addstring(
                        b,
                        to_luastring(String(Math.floor(date.getTime() / 1000))),
                    );
                    break;
...
const os_date = function (L: lua_State): number {
    let s = luaL_optlstring(L, 1, '%c') ?? to_luastring('%c');
...
const os_time = function (L: lua_State): number {
    let t;
    ...
    lua_pushinteger(L, Math.floor(t.getTime() / 1000));
    return 1;
};

const l_checktime = function (L: lua_State, arg: number): number {
    let t = luaL_checkinteger(L, arg);
    // luaL_argcheck(L, t, arg, "time out-of-bounds");
    return t;
};

const os_difftime = function (L: lua_State): number {
...
const os_setlocale = function (L: lua_State): number {
...
const syslib: Record<string, LuaCFunction> = {
    date: os_date,
...
syslib.clock = function (L: lua_State): number {
...
syslib.remove = function (L: lua_State): number {
    const filename = luaL_checkstring(L, 1);
    const path = to_jsstring(filename);
    const fs = requireModule('fs') as FsModule | null;
    if (fs) {
        try {
            fs.unlinkSync(path);
            ...
            lua_pushstring(L, to_luastring(error.message ?? String(e)));
            return 2;
        }
    }
    lua_pushnil(L);
...
syslib.rename = function (L: lua_State): number {
...
syslib.tmpname = function (L: lua_State): number {
    const os = requireModule('os') as OsModule | null;
    if (os) {
        try {
            const path =
...
syslib.execute = function (L: lua_State): number {
    lua_pushnil(L);
    lua_pushstring(L, to_luastring('os.execute is disabled'));
    return 2;
};

syslib.exit = function (L: lua_State): number {
    lua_pushnil(L);
    lua_pushstring(L, to_luastring('os.exit is not available'));
    return 2;
};

const luaopen_os = function (L: lua_State): number {
