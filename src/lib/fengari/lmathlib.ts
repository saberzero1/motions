...
let rand_state: number | undefined;
/* use same parameters as glibc LCG */
const l_rand = function (): number {
    rand_state = (1103515245 * (rand_state ?? 0) + 12345) & 0x7fffffff;
    return rand_state;
};
const l_srand = function (x: number): void {
    rand_state = x | 0;
    if (rand_state === 0) rand_state = 1;
};

const math_random = function (L: lua_State): number {
    let low = 0;
    let up = 0;
    ...
    switch (lua_gettop(L) /* check number of arguments */) {
        ...
        default:
            return luaL_error(L, to_luastring('wrong number of arguments'));
    }
...
const math_randomseed = function (L: lua_State): number {
    l_srand(luaL_checknumber(L, 1));
    l_rand(); /* discard first value to avoid undesirable correlations */
    return 0;
};

const math_abs = function (L: lua_State): number {
...
const math_sin = function (L: lua_State): number {
    lua_pushnumber(L, Math.sin(luaL_checknumber(L, 1)));
    return 1;
};

const math_cos = function (L: lua_State): number {
    lua_pushnumber(L, Math.cos(luaL_checknumber(L, 1)));
    return 1;
};

const math_tan = function (L: lua_State): number {
    lua_pushnumber(L, Math.tan(luaL_checknumber(L, 1)));
    return 1;
};

const math_asin = function (L: lua_State): number {
    lua_pushnumber(L, Math.asin(luaL_checknumber(L, 1)));
    return 1;
};

const math_acos = function (L: lua_State): number {
    lua_pushnumber(L, Math.acos(luaL_checknumber(L, 1)));
    return 1;
};

const math_atan = function (L: lua_State): number {
...
const math_toint = function (L: lua_State): number {
    let n = lua_tointegerx(L, 1);
    if (n !== false) lua_pushinteger(L, n);
    else {
        luaL_checkany(L, 1);
...
const pushnumint = function (L: lua_State, d: number): void {
...
const math_floor = function (L: lua_State): number {
    if (lua_isinteger(L, 1)) lua_settop(L, 1);
    else pushnumint(L, Math.floor(luaL_checknumber(L, 1)));

    return 1;
};

const math_ceil = function (L: lua_State): number {
    if (lua_isinteger(L, 1)) lua_settop(L, 1);
    else pushnumint(L, Math.ceil(luaL_checknumber(L, 1)));

    return 1;
};

const math_sqrt = function (L: lua_State): number {
    lua_pushnumber(L, Math.sqrt(luaL_checknumber(L, 1)));
    return 1;
};

const math_ult = function (L: lua_State): number {
...
const math_log = function (L: lua_State): number {
    let x = luaL_checknumber(L, 1);
    let res;
    if (lua_isnoneornil(L, 2)) res = Math.log(x);
    else {
        let base = luaL_checknumber(L, 2);
        if (base === 2) res = Math.log2(x);
        else if (base === 10) res = Math.log10(x);
        else res = Math.log(x) / Math.log(base);
    }
    lua_pushnumber(L, res);
    return 1;
};

const math_exp = function (L: lua_State): number {
    lua_pushnumber(L, Math.exp(luaL_checknumber(L, 1)));
    return 1;
};

const math_deg = function (L: lua_State): number {
    lua_pushnumber(L, luaL_checknumber(L, 1) * (180 / Math.PI));
    return 1;
};

const math_rad = function (L: lua_State): number {
    lua_pushnumber(L, luaL_checknumber(L, 1) * (Math.PI / 180));
    return 1;
};

const math_min = function (L: lua_State): number {
...
const math_max = function (L: lua_State): number {
    let n = lua_gettop(L); /* number of arguments */
...
const math_type = function (L: lua_State): number {
    if (lua_type(L, 1) === LUA_TNUMBER) {
        if (lua_isinteger(L, 1)) lua_pushliteral(L, 'integer');
        else lua_pushliteral(L, 'float');
    } else {
        luaL_checkany(L, 1);
        lua_pushnil(L);
    }
...
const math_fmod = function (L: lua_State): number {
    if (lua_isinteger(L, 1) && lua_isinteger(L, 2)) {
        let d = lua_tointeger(L, 2);
        /* no special case needed for -1 in javascript */
        if (d === 0) {
            luaL_argerror(L, 2, to_luastring('zero'));
        } else lua_pushinteger(L, lua_tointeger(L, 1) % d);
    } else {
...
const math_modf = function (L: lua_State): number {
    if (lua_isinteger(L, 1)) {
...
const mathlib: Record<string, LuaCFunction> = {
    abs: math_abs,
    acos: math_acos,
    asin: math_asin,
    atan: math_atan,
...
const luaopen_math = function (L: lua_State): number {
