...
const {
    ...
    LUA_MASKCOUNT,
    LUA_MASKLINE,
    constant_types: { LUA_TBOOLEAN, LUA_TNIL, LUA_TTABLE },
    thread_status: { LUA_ERRRUN, LUA_YIELD },
    from_userstring,
    luastring_eq,
    luastring_indexOf,
    to_luastring,
} = require('./defs.js');
const { api_check, lua_assert } = require('./llimits.js');
...
const currentline = function (ci) {
    return ci.func.value.p.lineinfo.length !== 0
        ? ci.func.value.p.lineinfo[currentpc(ci)]
        : -1;
};
            ...
            case OCi.OP_GETTABLE: {
                ...
                let vn =
                    i.opcode === OCi.OP_GETTABLE
                        ? lfunc.luaF_getlocalname(p, t + 1, pc)
                        : upvalname(p, t);
