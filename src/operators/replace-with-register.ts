...
export function replaceWithRegisterOperator(
    ...
    // Resolve the register to read from (default: unnamed register `"`)
    const regName = operatorArgs.registerName ?? '"';
    const rc = Vim.getRegisterController();
    const reg = rc.registers[regName];
    if (!reg) return;
    ...
    if (operatorArgs.linewise) {
        ...
        // Cursor lands at the beginning of the replacement (vim-ReplaceWithRegister
        // convention: land on the first character of the pasted text).
        return anchor;
    }
}
