...
| `keeps source-rendered frontmatter fully navigable`                        | macOS, Windows  | 2                       | **Cross-platform, product-side.** `frontmatter walk stalled: rows [7,6,6,6,6,6]` with `mode: "source"`, `propertiesInDocument: "visible"`, `metadataContainers: 1` — **byte-identical** on Windows (`36251726065`) to the macOS observation below. That confirms the prediction already recorded against it: the fault is the `setPropertiesSource()` gate reporting the wrong mode, and is not platform-specific. Not re-run. |
...
**Windows confirmed the prediction.** Run `36251726065` reproduced it on
`windows-latest` with the same diagnostics to the digit — `rows [7,6,6,6,6,6]`,
`cmRows [7,6,6,6,6,6]`, `mode: "source"`, `propertiesInDocument: "visible"`,
`metadataContainers: 1`. The sentence above guessed the gating callback would
not be macOS-specific; two platforms producing an identical stall sequence is
the evidence for that, and moves this from environment-suspected to
product-side. It is still open: nobody has forced the failure, which is what
rule 3 asks for before a fix.
