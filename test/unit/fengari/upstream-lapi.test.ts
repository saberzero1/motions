...
test('luaL_newstate, lua_pushnil, luaL_typename', () => {
    ...
    expect(lauxlib.luaL_typename(L, -1)).toEqual(to_luastring('nil'));
});
...
test('lua_pushnumber', () => {
    ...
    expect(lauxlib.luaL_typename(L, -1)).toEqual(to_luastring('number'));

    expect(lua.lua_tonumber(L, -1)).toBe(10.5);
});
...
test('lua_pushliteral', () => {
    ...
    expect(lauxlib.luaL_typename(L, -1)).toEqual(to_luastring('string'));
...
test('lua_pushboolean', () => {
    ...
    expect(lauxlib.luaL_typename(L, -1)).toEqual(to_luastring('boolean'));
...
test('lua_pushvalue', () => {
    ...
    expect(lauxlib.luaL_typename(L, -2)).toEqual(to_luastring('string'));
...
test('lua_pushjsclosure', () => {
    ...
    expect(lauxlib.luaL_typename(L, -1)).toEqual(to_luastring('function'));
});
...
test('lua_call (calling a light JS function)', () => {
    ...
    {
        let fn = function (callState: lua_State) {
            lua.lua_pushliteral(callState, 'hello');
            return 1;
        };
        lua.lua_pushjsfunction(L, fn);
        lua.lua_call(L, 0, 1);
...
test('lua_call (calling a JS closure)', () => {
...

    {
        let fn = function (callState: lua_State) {
            lua.lua_pushstring(
                callState,
                lua.lua_tostring(callState, lua.lua_upvalueindex(1)),
            );
            return 1;
        };
    ...
    expect(lua.lua_tojsstring(L, -1)).toBe('upvalue hello!');
});
...
test('lua_pcall that breaks', () => {
...

    {
        let fn = function (): number {
            return 'undefined_value' as unknown as number;
        };
        lua.lua_pushjsfunction(L, fn);
...
test('lua_load with no chunkname', () => {
...

    {
        lua.lua_load(
            L,
            function (_L: lua_State, data: unknown) {
                let s = data as { code: Uint8Array | null };
                let r = s.code;
                s.code = null;
                return r;
            },
...
test('lua_load and lua_call it', () => {
...

    {
        ...
        lua.lua_load(
            L,
            function (_L: lua_State, data: unknown) {
                let s = data as { bc: Uint8Array | null };
                let r = s.bc;
                s.bc = null;
                return r;
            },
            { bc: bc },
            to_luastring('test-lua_load'),
            to_luastring('binary'),
        );
        lua.lua_call(L, 0, 1);
    }

    expect(lua.lua_tojsstring(L, -1)).toBe('JS > Lua > JS \\o/');
});
...
test('lua_settable, lua_gettable', () => {
    ...
    expect(lua.lua_tojsstring(L, -1)).toBe('value');
});
...
describe('lua_atnativeerror', () => {
    ...
    test('native error handler returns string', () => {
        ...
        lua.lua_atnativeerror(L, function (errState: lua_State) {
            let e = lua.lua_touserdata(errState, 1);
            expect(e).toBe(errob);
            lua.lua_pushstring(errState, to_luastring('runtime error!'));
            return 1;
        });
        lua.lua_pushcfunction(L, function () {
            throw errob;
    ...
    test('native error handler rethrows lua error', () => {
        ...
        lua.lua_atnativeerror(L, function (errState: lua_State) {
            let e = lua.lua_touserdata(errState, 1);
            expect(e).toBe(errob);
            lauxlib.luaL_error(errState, to_luastring('runtime error!'));
            return 1;
        });
        lua.lua_pushcfunction(L, function () {
            throw errob;
