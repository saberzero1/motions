...
test('debug.sethook', () => {
    ...
    expect(lua.lua_tojsstring(L, -1)).toBe(
...
test('debug.gethook', () => {
    ...
    expect(lua.lua_typename(L, lua.lua_type(L, -3))).toEqual(
...
test('debug.upvaluejoin', () => {
    ...
    expect(lua.lua_tojsstring(L, -1)).toBe('upvalue2');
});
...
test('debug.traceback (with a global)', () => {
    ...
    `;
    {
        ...
        lauxlib.luaL_loadbuffer(
            ...
            luaBytes,
            luaBytes.length,
            to_luastring('traceback-test'),
        );
        lua.lua_call(L, 0, -1);
    }

    expect(lua.lua_tojsstring(L, -1)).toBe(`stack traceback:
