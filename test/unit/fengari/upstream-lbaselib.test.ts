...
test('rawset, rawget', () => {
    ...
    expect(lua.lua_tojsstring(L, -3)).toBe('hello');

    expect(lua.lua_tojsstring(L, -2)).toBe('bye');

    expect(lua.lua_tojsstring(L, -1)).toBe('bye');
});
...
test('type', () => {
    ...
    expect(lua.lua_tojsstring(L, -5)).toBe('number');

    expect(lua.lua_tojsstring(L, -4)).toBe('boolean');

    expect(lua.lua_tojsstring(L, -3)).toBe('string');

    expect(lua.lua_tojsstring(L, -2)).toBe('table');

    expect(lua.lua_tojsstring(L, -1)).toBe('nil');
});
...
test('select', () => {
    ...
    const firstTable = lua.lua_topointer(L, -3) as {
        strong: Map<unknown, { value: { value: number } }>;
    };
    const secondTable = lua.lua_topointer(L, -2) as {
        strong: Map<unknown, { value: { value: number } }>;
    };
    const thirdTable = lua.lua_topointer(L, -1) as {
        strong: Map<unknown, { value: { value: number } }>;
    };

    expect(
        [...firstTable.strong.entries()].map((e) => e[1].value.value),
    ).toEqual([3]);

    expect(
        [...secondTable.strong.entries()].map((e) => e[1].value.value).sort(),
    ).toEqual([2, 3]);

    expect(
        [...thirdTable.strong.entries()].map((e) => e[1].value.value).sort(),
    ).toEqual([2, 3]);
});
